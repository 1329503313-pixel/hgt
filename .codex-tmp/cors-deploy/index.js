...
import { findHighlySimilarSoup } from "./soupSimilarity.js";
import { canEnableAiGameRole, canViewAllSoupContentRole, hasUnlimitedSoupPublishingRole, isBackofficeAdminRole, isSuperAdminRole, normalizeUserRole } from "./roles.js";
...
import { runDailyInviteShellSettlement, setInviteRewardProgressListener, syncPendingInviteEmailRewards } from "./inviteRewards.js";
...
import { evaluationCountsTowardScore, scoringEvaluationJoin, scoringEvaluationPredicate } from "./evaluationScoring.js";
import { idempotentSoupId, isValidIdempotencyKey } from "./idempotency.js";
import { homeSoupCategoryOrder, homeSoupCategoryRequiresAuth, parseHomeSoupCategory } from "./homeSoupCategory.js";
import { RANKING_REWARD_BOARD_LABELS, runRankingRewardScheduler } from "./rankingRewards.js";
import { AUTH_COOKIE_NAME, LEGACY_AUTH_COOKIE_NAME, authTokenFromCookieHeader, authTokenFromCookies } from "./authCookies.js";
import { LEGENDARY_CARD_DRAW_COUNT_SQL, SYSTEM_BADGE_ACHIEVEMENT_POINTS, badgeUnlockNotificationContent, calculateBadgeShellReward, systemBadgeKeysWithPrerequisites } from "./badgeRewards.js";
import { adjustShellBalance, awardBeginnerTask, bulkAdjustShellBalances, awardShellTask, beijingTaskDate, isEligibleCircleTaskMessageType, setShellBadgeProgressListener, setTaskGiftNotificationListener, shellTaskCenter, shellTransactions } from "./shellCurrency.js";
import { flushUserBehaviorAnalytics, initializeUserBehaviorAnalytics, recordUserBehavior, USER_BEHAVIOR_DEFINITIONS } from "./behaviorAnalytics.js";
import { WebSocket, WebSocketServer } from "ws";
import onlineSoupRouter, { cleanupOnlineSoupInactiveHostRooms, cleanupOnlineSoupStaleSeats, ONLINE_SOUP_PARTICIPANT_CAPACITY, ONLINE_SOUP_PLAYER_CAPACITY, setOnlineSoupEventEmitter, setOnlineSoupLobbyEventEmitter, validRoomInviteToken } from "./onlineSoup.js";
...
const onlineSoupRoomSocketClients = new Map();
const onlineSoupLobbySocketClients = new Set();
const circleSocketClients = new Map();
const presenceConnectionCounts = new Map();
const visiblyOnlineUsers = new Set();
const presenceOfflineTimers = new Map();
function emitOnlineSoupSocketEvent(roomId, event, payload) {
    ...
    for (const client of clients) {
        if (client.readyState !== WebSocket.OPEN)
            continue;
        client.send(message);
        if (kickedUserId && String(client.onlineSoupUserId) === kickedUserId) {
            client.close(4003, "removed from room");
        }
    }
}
function emitCircleSocketEvent(circleId, event, payload) {
...
setOnlineSoupLobbyEventEmitter((event, payload) => {
    if (!onlineSoupLobbySocketClients.size)
        return;
    const message = JSON.stringify({ event, payload });
    for (const client of onlineSoupLobbySocketClients)
        if (client.readyState === WebSocket.OPEN)
            client.send(message);
});
...
function emitUnreadChanged(userId, source) {
...
function broadcastPresenceChanged(userId, online) {
...
function registerPresenceConnection(userId) {
...
function unregisterPresenceConnection(userId) {
    ...
    if (presenceOfflineTimers.has(userId))
        return;
    const timer = setTimeout(() => {
        presenceOfflineTimers.delete(userId);
        if ((presenceConnectionCounts.get(userId) ?? 0) > 0 || !visiblyOnlineUsers.delete(userId))
            return;
...
async function emitCircleMemberPresence(userId, online) {
    const [rows] = await pool.query(`SELECT changed.circle_id, recipient.user_id AS recipient_id
     FROM circle_members changed
     INNER JOIN circle_members recipient ON recipient.circle_id = changed.circle_id
     WHERE changed.user_id = ?`, [userId]);
    const membersByCircle = new Map();
    for (const row of rows) {
        const circleId = String(row.circle_id);
        const members = membersByCircle.get(circleId) ?? new Set();
...
async function broadcastUnreadChanged(source) {
    const [rows] = await pool.query("SELECT id FROM users");
    for (const row of rows)
        emitUnreadChanged(String(row.id), source);
}
function createRateLimiter(windowMs, maxRequests, keyFor) {
...
function clearCookieVariants(res, name) {
...
function extractAuthClaims(req) {
    // 方式 1: Cookie 中的 JWT
...
app.get("/api/app/android-update", async (req, res) => {
    ...
    const [rows] = await pool.query(`SELECT id, version_code, version_name, min_supported_version_code, apk_url,
...
const ACTIVITY_CONDITION_SOURCES = {
...
function badgeActivityConditions(value) {
    try {
...
function specialBadgeTier(value) {
    return String(value) === "epic" ? "epic" : "legend";
}
const BADGE_OWNERSHIP_REFRESH_MS = 60 * 60 * 1000;
let badgeOwnershipRates = {};
let equippedSpecialBadgeMetadata = {};
async function refreshEquippedSpecialBadgeMetadata() {
    const [rows] = await pool.query("SELECT id, name, tier FROM legendary_badges");
...
async function refreshBadgeOwnershipRates() {
    const [[[totalRow]], [ownerRows]] = await Promise.all([
        pool.query("SELECT COUNT(*) AS total FROM users WHERE role IN ('user', 'vip')"),
        pool.query(`SELECT ubu.badge_key, COUNT(DISTINCT ubu.user_id) AS owner_count
...
const soupSchema = z.object({
    ...
    coverImage: z
        ...
        .optional()
        .default("")
        .refine((value) => !value || /^data:image\/(png|jpeg);base64,/.test(value) || /^\/api\/media\/soups\/[^/]+\/cover$/.test(value), "封面仅支持 JPG 或 PNG"),
...
function feedbackPayload(row, includeScreenshot = false) {
    return {
        id: String(row.id),
        title: String(row.title),
...
function avatarUrl(userId, stored, hasAvatar = false) {
    if (!stored && !hasAvatar)
        return null;
    return `/api/media/users/${encodeURIComponent(String(userId))}/avatar`;
}
function profileBackgroundUrl(userId, hasBackground, updatedAt) {
    if (!bool(hasBackground))
        return null;
...
function soupImageUrl(soupId, stored, variant, hasStored = false) {
    if (!stored && !hasStored)
        return null;
...
function circleAvatarUrl(circleId, stored, updatedAt, hasStored = false) {
    if (!stored && !hasStored)
        return "/turtle-avatar.png?v=5272-20260716";
    const value = stored ? String(stored) : "";
    if (value && !value.startsWith("data:image/"))
        return publicOssUrl(value);
    const version = value ? createHash("sha1").update(value).digest("hex").slice(0, 16) : new Date(updatedAt).getTime();
...
async function sendStoredImage(req, res, value, maxWidth, cacheControl = "public, max-age=3600, stale-while-revalidate=86400", fallbackPath) {
    ...
    res.setHeader("Cache-Control", cacheControl);
    if (reqHeaderMatches(req.headers["if-none-match"], etag))
    ...
    if (!output) {
        ...
        if (optimizedImageCache.size > 200)
            optimizedImageCache.delete(optimizedImageCache.keys().next().value);
    }
    res.setHeader("Content-Type", "image/webp");
    res.setHeader("Content-Length", output.length);
    return res.send(output);
}
async function optimizeCircleAvatar(base64, circleId) {
    const decoded = decodeDataImage(base64);
    if (!decoded || decoded.buffer.length > 6 * 1024 * 1024)
        return null;
    try {
...
async function optimizeFeedbackScreenshot(base64, feedbackId) {
    const decoded = decodeDataImage(base64);
    if (!decoded || decoded.buffer.length > 5 * 1024 * 1024)
        return null;
    try {
        const output = await sharp(decoded.buffer)
            .rotate()
...
function reqHeaderMatches(header, etag) {
...
const currentUserInFlight = new Map();
async function currentUser(req) {
    ...
    if (pending)
        return pending;
    const request = (async () => {
        const [rows] = await pool.query(`SELECT id, username, nickname, role, token_version, created_at, experience,
         equipped_badge_key, equipped_badge_icon_url, avatar IS NOT NULL AS has_avatar
       FROM users WHERE id = ? LIMIT 1`, [claims.id]);
        const row = rows[0];
        ...
        if (currentUserCache.size > 1000)
            currentUserCache.delete(currentUserCache.keys().next().value);
        return user;
    })();
...
async function requireBackofficeAdmin(req, res) {
    const user = await requireAuth(req, res);
    if (!user)
        return null;
...
async function getSoupPublishUsage(userId) {
    const date = beijingDateString();
    const [rows] = await pool.query("SELECT published_count, auto_reject_count FROM soup_publish_daily_usage WHERE user_id = ? AND usage_date = ? LIMIT 1", [userId, date]);
    return {
        date,
...
async function recordSoupAutoReject(userId, date) {
    await pool.query(`INSERT INTO soup_publish_daily_usage (user_id, usage_date, auto_reject_count)
     VALUES (?, ?, 1)
     ON DUPLICATE KEY UPDATE auto_reject_count = auto_reject_count + 1`, [userId, date]);
}
function toUser(row) {
    return {
        id: row.id,
...
function withoutPrivateUsername(user) {
    const { username: _username, tokenVersion: _tokenVersion, ...publicUser } = user;
    return publicUser;
}
...
function equippedBadge(key, iconUrl) {
    if (!key || !iconUrl)
        return null;
    const badgeKey = String(key);
    const special = equippedSpecialBadgeMetadata[badgeKey];
    if (special)
        return { key: badgeKey, iconUrl: String(iconUrl), ...special };
...
async function ownedBadgeIconUrl(userId, badgeKey) {
    const [[unlock]] = await pool.query("SELECT badge_key FROM user_badge_unlocks WHERE user_id = ? AND badge_key = ? LIMIT 1", [userId, badgeKey]);
    if (!unlock)
        return null;
    if (badgeKey.startsWith("legendary:")) {
        const [[badge]] = await pool.query("SELECT icon_url FROM legendary_badges WHERE id = ? LIMIT 1", [badgeKey.slice("legendary:".length)]);
...
function mapEvaluation(row, canViewHiddenContent = true, creatorId) {
...
async function getSoupRaw(id) {
    const [rows] = await pool.query("SELECT * FROM soups WHERE id = ? LIMIT 1", [id]);
    return rows[0] ?? null;
}
async function getSoupSummariesWhere(whereSql, params) {
    const [rows] = await pool.query(`SELECT ${soupSummaryColumns("s")}, NULL AS creator_avatar, u.avatar IS NOT NULL AS creator_has_avatar,
...
async function getCreatorCertificationSoups(userId) {
    return getSoupSummariesWhere("s.creator_id = ?", [userId]);
}
function isQualificationSoup(soup) {
    return soup.isOriginal && soup.reviewStatus === "approved" && soup.heatValue >= 3000 && (soup.averageTotal ?? 0) >= 3.5;
}
function isPrimaryCertificationSoup(soup) {
    return isQualificationSoup(soup) && soup.heatValue >= 5000 && (soup.averageTotal ?? 0) >= 4;
}
async function getExcellentAuthorApplicationDetail(applicationId) {
    const [[application]] = await pool.query("SELECT * FROM excellent_author_applications WHERE id = ? LIMIT 1", [applicationId]);
    if (!application)
        return null;
    const [selectedRows] = await pool.query("SELECT soup_id FROM excellent_author_application_soups WHERE application_id = ? ORDER BY sort_order ASC", [applicationId]);
    ...
    return {
        ...
        qualificationSoups: selectedIds
            .map((id) => byId.get(id))
            .filter((soup) => Boolean(soup))
    };
}
async function findDuplicateSoup(input, excludedId) {
    const [rows] = await pool.query(`SELECT id, title, surface, bottom
     FROM soups
     ${excludedId ? "WHERE id <> ?" : ""}`, excludedId ? [excludedId] : []);
    return findHighlySimilarSoup(input, rows.map((row) => ({
        id: String(row.id),
        title: String(row.title),
...
async function canViewFull(soup, user) {
    if (bool(soup.is_bottom_public))
        return true;
    if (!user)
        return false;
    if (canViewAllSoupContentRole(user.role) || user.id === soup.creator_id)
        return true;
    const [rows] = await pool.query("SELECT id FROM soup_access_grants WHERE soup_id = ? AND user_id = ? LIMIT 1", [soup.id, user.id]);
    return rows.length > 0;
}
function canSeeSoupSurface(soup, user) {
    ...
    if (bool(soup.is_surface_public))
        return true;
    if (!user)
        return false;
    return canViewAllSoupContentRole(user.role) || user.id === soup.creator_id;
}
async function notify(userId, type, title, content, relatedId, actorId = null) {
    const [result] = await pool.query("INSERT IGNORE INTO notifications (id, user_id, type, title, content, related_id, actor_id) VALUES (?, ?, ?, ?, ?, ?, ?)", [nanoid(), userId, type, title.slice(0, 120), content.slice(0, 500), relatedId, actorId]);
    if (result.affectedRows > 0)
        emitUnreadChanged(userId, type);
}
async function recordAdminModuleEvent(moduleKey) {
    const [result] = await pool.query("INSERT INTO admin_module_events (module_key) VALUES (?)", [moduleKey]);
...
async function grantBadgeWithConnection(connection, options) {
    const [unlockResult] = await connection.query("INSERT IGNORE INTO user_badge_unlocks (user_id, badge_key, surfaced_at) VALUES (?, ?, NULL)", [options.userId, options.badgeKey]);
    ...
    const [rewardHistoryResult] = await connection.query(`INSERT IGNORE INTO badge_shell_reward_history
      (user_id, badge_key, achievement_points_snapshot, shell_reward, settlement_status, reward_source)
     VALUES (?, ?, ?, 0, 'settled', 'realtime')`, [options.userId, options.badgeKey, achievementPoints]);
    const shellReward = calculateBadgeShellReward(achievementPoints, options.rewardEligible !== false, rewardHistoryResult.affectedRows > 0);
    let balanceAfter = null;
    if (shellReward > 0) {
        const [[userRow]] = await connection.query("SELECT shell_balance FROM users WHERE id = ? FOR UPDATE", [options.userId]);
        if (!userRow)
            throw new Error("BADGE_REWARD_USER_NOT_FOUND");
       ...
       VALUES (?, ?, 'badge_unlock_reward', ?, ?, 'badge', ?, ?, ?)`, [
        ...
        await connection.query(`UPDATE badge_shell_reward_history
       SET shell_reward = ?
       WHERE user_id = ? AND badge_key = ?`, [shellReward, options.userId, options.badgeKey]);
    }
    let notificationCreated = false;
    if (options.notification) {
        const notification = options.notification;
        const [notificationResult] = await connection.query(`INSERT IGNORE INTO notifications
...
async function grantBadge(options) {
    ...
    try {
        ...
        if (result.notificationCreated)
            emitUnreadChanged(options.userId, "badge_unlock");
        if (result.shellReward > 0)
            queueSystemBadgeSync([options.userId]);
        return result;
    }
    catch (error) {
...
async function backofficeAdminIds() {
    const [rows] = await pool.query("SELECT id FROM users WHERE role IN ('super_admin', 'backoffice_admin')");
    return rows.map((row) => String(row.id));
}
async function recordLoginDay(userId) {
    const taskDate = beijingTaskDate();
    const [[loginResult]] = await Promise.all([
        pool.query(`INSERT IGNORE INTO user_login_days (user_id, login_date)
       VALUES (?, DATE(DATE_ADD(UTC_TIMESTAMP(), INTERVAL 8 HOUR)))`, [userId]),
...
const queuedLoginDayRecords = new Map();
function queueLoginDayRecord(userId) {
    const key = `${userId}:${beijingTaskDate()}`;
    if ((queuedLoginDayRecords.get(key) ?? 0) > Date.now())
        return;
    ...
    if (queuedLoginDayRecords.size > 1000)
        queuedLoginDayRecords.delete(queuedLoginDayRecords.keys().next().value);
}
...
const BADGE_NOTIFICATION_LABELS = {
...
function badgeNotificationLabel(key) {
...
async function getMaxOriginalSoupHeat(userId) {
    const [[row]] = await pool.query(`SELECT COALESCE(MAX(
     ...
     FROM soups s
     WHERE s.creator_id = ? AND s.is_original = TRUE`, [userId]);
    return Math.round(Number(row?.max_heat ?? 0));
}
const achievementStatsCache = new Map();
async function getAchievementStats(userId) {
    const cached = achievementStatsCache.get(userId);
    if (cached && cached.expiresAt > Date.now())
        return cached.stats;
    const [[soupRows], [favRows], [evalRows], [likeRows], [keyHitRows], [loginDayRows], [receivedLikeRows], [receivedFavoriteRows], [receivedCommentRows], [writtenCommentRows], [aiCompletionRows], maxOriginalSoupHeat, [assetSummaryRows], [drawTotalRows], [legendaryCardDrawRows], [threeStarRows], [completePackRows], [shellRows]] = await Promise.all([
        pool.query("SELECT COUNT(*) AS count FROM soups WHERE creator_id = ?", [userId]),
        pool.query("SELECT COUNT(*) AS count FROM soup_favorites WHERE user_id = ?", [userId]),
        pool.query("SELECT COUNT(DISTINCT soup_id) AS count FROM evaluations WHERE reviewer_id = ?", [userId]),
        pool.query("SELECT COUNT(*) AS count FROM soup_likes WHERE user_id = ?", [userId]),
        pool.query("SELECT COUNT(*) AS count FROM game_key_hits WHERE user_id = ?", [userId]),
        pool.query("SELECT COUNT(*) AS count FROM user_login_days WHERE user_id = ?", [userId]),
        pool.query("SELECT COUNT(*) AS count FROM soup_like_history WHERE creator_id = ?", [userId]),
        pool.query("SELECT COUNT(*) AS count FROM soup_favorite_history WHERE creator_id = ?", [userId]),
        pool.query("SELECT COUNT(*) AS count FROM evaluation_comment_history WHERE creator_id = ? AND is_original = TRUE", [userId]),
        pool.query("SELECT COUNT(*) AS count FROM evaluation_comment_history WHERE reviewer_id = ?", [userId]),
        pool.query("SELECT COUNT(*) AS count FROM game_completions WHERE user_id = ?", [userId]),
        getMaxOriginalSoupHeat(userId),
        pool.query(`SELECT COALESCE(total_collection_value, 0) AS total_collection_value,
        COALESCE(unlocked_card_count, 0) AS unlocked_card_count
       FROM user_asset_summaries WHERE user_id = ? LIMIT 1`, [userId]),
        pool.query("SELECT total_draw_count FROM user_asset_draw_totals WHERE user_id = ? LIMIT 1", [userId]),
        pool.query(LEGENDARY_CARD_DRAW_COUNT_SQL, [userId]),
        pool.query(`SELECT
       ...
       FROM (
         ...
         GROUP BY pack.id
       ) pack_progress`, [userId]),
        pool.query(`SELECT u.shell_balance, u.generosity_value, u.charm_value,
       ...
       FROM users u WHERE u.id = ? LIMIT 1`, [userId])
    ]);
    const stats = {
...
async function activityConditionCount(userId, condition) {
    ...
    const sql = `SELECT COUNT(*) AS count FROM ${source.table} activity_source WHERE activity_source.${source.userColumn} = ?${longTerm ? "" : ` AND ${source.dateExpression("activity_source")} BETWEEN ? AND ?`}`;
    const params = longTerm ? [userId] : [userId, condition.startDate, condition.endDate];
    const [[row]] = await pool.query(sql, params);
    return Number(row?.count ?? 0);
}
async function usersMatchingActivityConditions(conditions) {
    ...
    const clauses = conditions.map((condition, index) => {
        ...
        const longTerm = condition.startDate === "long_term" || condition.endDate === "long_term";
        if (!longTerm)
            params.push(condition.startDate, condition.endDate);
        params.push(["login", "user_joined"].includes(condition.kind) ? 1 : (condition.target ?? 1));
        return `(SELECT COUNT(*) FROM ${source.table} ${alias} WHERE ${alias}.${source.userColumn} = u.id${longTerm ? "" : ` AND ${source.dateExpression(alias)} BETWEEN ? AND ?`}) >= ?`;
    });
    const [rows] = await pool.query(`SELECT u.id FROM users u WHERE u.role = 'user' AND ${clauses.join(" AND ")} ORDER BY u.created_at ASC, u.id ASC`, params);
    return rows.map((row) => String(row.id));
}
const activityBadgeSyncCache = new Map();
async function syncActivityBadges(userId) {
    const cached = activityBadgeSyncCache.get(userId);
    if (cached && cached.expiresAt > Date.now())
        return cached.earned;
    const [badgeRows] = await pool.query("SELECT id, name, achievement_points, activity_conditions FROM legendary_badges WHERE badge_type = 'activity' AND activity_conditions IS NOT NULL");
    ...
    const [unlockRows] = await pool.query("SELECT badge_key FROM user_badge_unlocks WHERE user_id = ?", [userId]);
    const owned = new Set(unlockRows.map((row) => String(row.badge_key)));
    const [grantHistoryRows] = await pool.query("SELECT badge_id FROM activity_badge_grant_history WHERE user_id = ?", [userId]);
    const previouslyGranted = new Set(grantHistoryRows.map((row) => String(row.badge_id)));
    const earned = [];
    for (const badge of badgeRows) {
        const key = `legendary:${badge.id}`;
        if (owned.has(key) || previouslyGranted.has(String(badge.id)))
            continue;
        ...
        if (conditions.every((condition, index) => counts[index] >= (["login", "user_joined"].includes(condition.kind) ? 1 : (condition.target ?? 1)))) {
            ...
            try {
                await connection.beginTransaction();
                const [historyResult] = await connection.query("INSERT IGNORE INTO activity_badge_grant_history (user_id, badge_id) VALUES (?, ?)", [userId, badge.id]);
                ...
                await connection.commit();
                if (grantResult?.notificationCreated)
                    emitUnreadChanged(userId, "badge_unlock");
                if ((grantResult?.shellReward ?? 0) > 0)
                    queueSystemBadgeSync([userId]);
                if (grantResult?.granted)
                    earned.push({ key, name: String(badge.name) });
            }
            catch (error) {
                await connection.rollback();
                throw error;
...
async function syncSystemBadgeUnlocks(userId, rewardEligible = true) {
    ...
    const qualifiedKeys = systemBadgeKeysWithPrerequisites(earnedKeys, unlocked, BADGE_THRESHOLDS.map((badge) => badge.key));
...
async function markPendingBadgePopupsSurfaced(userId) {
    await pool.query("UPDATE user_badge_unlocks SET surfaced_at = CURRENT_TIMESTAMP WHERE user_id = ? AND surfaced_at IS NULL", [userId]);
}
async function claimPendingBadgePopups(userId) {
    const connection = await pool.getConnection();
    try {
        await connection.beginTransaction();
        const [rows] = await connection.query(`SELECT badge_key
       ...
       FOR UPDATE`, [userId]);
        ...
        if (keys.length > 0) {
            ...
            await connection.query(`UPDATE user_badge_unlocks
         SET surfaced_at = CURRENT_TIMESTAMP
         WHERE user_id = ? AND surfaced_at IS NULL AND badge_key IN (${placeholders})`, [userId, ...keys]);
        }
        await connection.commit();
        return keys;
    }
...
async function getLegendaryBadgeUnlockDetails(userId, keys) {
    ...
    const [rows] = await pool.query(`SELECT lb.id, lb.name, lb.description, lb.requirement, lb.icon_url,
     ...
     WHERE ubu.user_id = ? AND ubu.badge_key IN (${placeholders})`, [userId, ...legendaryKeys]);
    const byKey = new Map(rows.map((row) => [`legendary:${row.id}`, {
            ...
            badgeType: String(row.badge_type ?? "achievement"),
...
const pendingBadgeSyncUsers = new Set();
let badgeSyncTimer = null;
function queueSystemBadgeSync(userIds) {
    ...
    badgeSyncTimer = setTimeout(async () => {
        ...
        for (let index = 0; index < batch.length; index += 5) {
            await Promise.all(batch.slice(index, index + 5).map(async (userId) => {
                try {
                    const [rows] = await pool.query("SELECT badges_initialized FROM users WHERE id = ? LIMIT 1", [userId]);
                    if (rows.length === 0)
                        return;
...
const pendingActivityBadgeSyncUsers = new Set();
let activityBadgeSyncTimer = null;
function queueActivityBadgeSync(userIds) {
    userIds.filter(Boolean).forEach((userId) => {
        activityBadgeSyncCache.delete(userId);
        pendingActivityBadgeSyncUsers.add(userId);
    });
    if (activityBadgeSyncTimer)
        return;
...
async function optimizeCoverImage(base64, soupId) {
    try {
...
app.post("/api/auth/register", registerRateLimiter, async (req, res) => {
    const parsed = z
        ...
        .safeParse(req.body);
    if (!parsed.success)
        return sendError(res, 400, parsed.error.issues[0]?.message ?? "注册信息不完整");
    ...
    if (isAdminRelatedNickname(nickname))
    ...
    if (exists.length)
        return sendError(res, 409, "账号已存在");
    const id = nanoid();
    ...
    try {
        ...
        let inviterId = null;
        if (invitationCode) {
            const [[inviter]] = await connection.query("SELECT id FROM users WHERE invite_code = ? LIMIT 1", [invitationCode]);
            if (!inviter) {
                await connection.rollback();
        ...
        for (let attempt = 0; attempt < 20; attempt += 1) {
            ...
            const [codeRows] = await connection.query("SELECT id FROM users WHERE invite_code = ? LIMIT 1", [ownInviteCode]);
            if (!codeRows.length)
                break;
            ownInviteCode = "";
        }
        ...
        await connection.query("INSERT INTO users (id, username, password, nickname, invite_code, role) VALUES (?, ?, ?, ?, ?, 'user')", [id, username, hash, nickname, ownInviteCode]);
        if (inviterId) {
            await connection.query("INSERT INTO user_invite_bindings (invitee_user_id, inviter_user_id, invite_code) VALUES (?, ?, ?)", [id, inviterId, invitationCode]);
            await connection.query(`INSERT INTO user_invite_reward_progress
          (invitee_user_id, inviter_user_id, settled_through)
         VALUES (?, ?, UTC_TIMESTAMP())`, [id, inviterId]);
        }
        await connection.commit();
    }
    catch (error) {
    ...
    const user = { id, username, nickname, avatar: null, role: "user", createdAt: new Date().toISOString(), level: 0, equippedBadge: null };
...
app.post("/api/auth/login", loginRateLimiter, async (req, res) => {
    ...
    if (!parsed.success)
        return sendError(res, 400, "请输入账号和密码");
    const [rows] = await pool.query(`SELECT id, username, password, nickname, role, token_version, created_at, experience,
       equipped_badge_key, equipped_badge_icon_url, avatar IS NOT NULL AS has_avatar
...
app.post("/api/auth/password", async (req, res) => {
    ...
    const parsed = z
        ...
        .safeParse(req.body);
    if (!parsed.success)
        return sendError(res, 400, parsed.error.issues[0]?.message ?? "密码信息不正确");
    const [rows] = await pool.query("SELECT id, token_version FROM users WHERE id = ? LIMIT 1", [user.id]);
    const row = rows[0];
    if (!row)
    ...
    await pool.query("UPDATE password_reset_tokens SET consumed_at = UTC_TIMESTAMP() WHERE user_id = ? AND consumed_at IS NULL", [user.id]);
...
const storedAvatarCache = new Map();
app.get("/api/media/users/:id/avatar", async (req, res) => {
    const cached = storedAvatarCache.get(req.params.id);
    if (cached && cached.expiresAt > Date.now())
        return sendStoredImage(req, res, cached.value, 160);
    const [[row]] = await pool.query("SELECT avatar FROM users WHERE id = ? LIMIT 1", [req.params.id]);
    if (!row)
        return sendError(res, 404, "用户不存在");
    storedAvatarCache.set(req.params.id, { expiresAt: Date.now() + 5 * 60_000, value: row.avatar });
    if (storedAvatarCache.size > 200)
        storedAvatarCache.delete(storedAvatarCache.keys().next().value);
...
app.get("/api/media/users/:id/profile-background", async (req, res) => {
    const [[row]] = await pool.query("SELECT profile_background FROM users WHERE id = ? LIMIT 1", [req.params.id]);
    if (!row)
        return sendError(res, 404, "用户不存在");
...
app.get("/api/media/circles/:id/avatar", async (req, res) => {
    const [[row]] = await pool.query("SELECT avatar FROM circles WHERE id = ? LIMIT 1", [req.params.id]);
    if (!row)
        return sendError(res, 404, "圈子不存在");
...
app.get("/api/media/soups/:id/thumbnail", async (req, res) => {
    const [[row]] = await pool.query("SELECT cover_thumbnail, review_status, creator_id FROM soups WHERE id = ? LIMIT 1", [req.params.id]);
    if (!row)
...
app.get("/api/media/soups/:id/cover", async (req, res) => {
    const [[row]] = await pool.query("SELECT cover_image, review_status, creator_id FROM soups WHERE id = ? LIMIT 1", [req.params.id]);
    if (!row)
        return sendError(res, 404, "作品不存在");
    const viewer = row.review_status === "approved" ? null : await currentUser(req);
    if (row.review_status !== "approved" && !isBackofficeAdminRole(viewer?.role) && viewer?.id !== String(row.creator_id)) {
...
app.get("/api/me/invited-users", async (req, res) => {
    ...
    const [totalRows, rows] = await Promise.all([
        pool.query("SELECT COUNT(*) AS total FROM user_invite_bindings WHERE inviter_user_id = ?", [user.id]).then(([result]) => result),
        pool.query(`SELECT invited.id, invited.nickname, invited.experience, invited.created_at,
       ...
       LIMIT ? OFFSET ?`, [user.id, limit, offset]).then(([result]) => result)
    ]);
    res.json({
        users: rows.map((row) => ({
            id: String(row.id),
...
app.get("/api/events", async (req, res) => {
    ...
    res.flushHeaders();
    const clients = userEventClients.get(user.id) ?? new Set();
    clients.add(res);
    ...
    req.on("close", () => {
        ...
        clients.delete(res);
        if (clients.size === 0)
            userEventClients.delete(user.id);
...
app.patch("/api/me/avatar", async (req, res) => {
    ...
    if (avatar) {
        ...
        if (!decoded)
            return sendError(res, 400, "头像格式不正确");
    ...
    if (avatar)
        await awardBeginnerTask(user.id, "upload_avatar");
...
app.get("/api/me/content-counts", async (req, res) => {
    ...
    const [[row]] = await pool.query(`SELECT
       ...
       (SELECT COUNT(*) FROM soup_likes lk INNER JOIN soups s ON s.id = lk.soup_id WHERE lk.user_id = ? AND s.review_status = 'approved') AS like_count`, [user.id, user.id, user.id]);
    res.json({
...
app.get("/api/me/soups", async (req, res) => {
    ...
    const [[totalRow]] = await pool.query("SELECT COUNT(*) AS total FROM soups WHERE creator_id = ?", [user.id]);
...
app.get("/api/me/excellent-author-application", async (req, res) => {
    ...
    const [[application]] = await pool.query(`SELECT id, status, created_at, handled_at
     FROM excellent_author_applications
     WHERE applicant_id = ?
     ORDER BY created_at DESC
     LIMIT 1`, [user.id]);
    const [[badge]] = await pool.query("SELECT badge_key FROM user_badge_unlocks WHERE user_id = ? AND badge_key = 'excellentAuthor:epic' LIMIT 1", [user.id]);
    res.json({
...
app.post("/api/me/excellent-author-application", async (req, res) => {
    ...
    if (!parsed.success)
        return sendError(res, 400, "请按要求选择5篇资格汤和1篇认证汤");
    const qualificationSoupIds = [...new Set(parsed.data.qualificationSoupIds)];
    if (qualificationSoupIds.length !== 5)
        return sendError(res, 400, "资格汤必须选择5篇，不能多也不能少");
    if (!qualificationSoupIds.includes(parsed.data.primarySoupId))
        return sendError(res, 400, "认证汤必须从5篇资格汤中选择");
    const soups = await getCreatorCertificationSoups(user.id);
    const byId = new Map(soups.map((soup) => [soup.id, soup]));
    ...
    try {
        const [[lock]] = await connection.query("SELECT GET_LOCK(?, 5) AS acquired", [`excellent-author:${user.id}`]);
        lockAcquired = Number(lock?.acquired) === 1;
        if (!lockAcquired)
            return sendError(res, 503, "申请提交繁忙，请稍后再试");
        const [existing] = await connection.query("SELECT status FROM excellent_author_applications WHERE applicant_id = ? AND status IN ('pending','approved') LIMIT 1", [user.id]);
        if (existing.some((row) => row.status === "approved"))
            return sendError(res, 409, "你已通过优秀作者认证");
        if (existing.some((row) => row.status === "pending"))
            return sendError(res, 409, "已有待审核的优秀作者认证申请");
        const id = nanoid();
        await connection.beginTransaction();
        await connection.query("INSERT INTO excellent_author_applications (id, applicant_id, applicant_name, primary_soup_id) VALUES (?, ?, ?, ?)", [id, user.id, user.nickname, parsed.data.primarySoupId]);
        for (const [index, soupId] of qualificationSoupIds.entries()) {
            await connection.query("INSERT INTO excellent_author_application_soups (application_id, soup_id, sort_order) VALUES (?, ?, ?)", [id, soupId, index]);
        }
        await connection.commit();
        await recordAdminModuleEvent("approvals");
        await Promise.all((await adminIds()).map((adminId) => notify(adminId, "excellent_author_application", "新的优秀作者认证申请", `${user.nickname} 提交了优秀作者认证申请`, id, user.id)));
        ...
        if (lockAcquired)
            await connection.query("SELECT RELEASE_LOCK(?)", [`excellent-author:${user.id}`]).catch(() => { });
        connection.release();
    }
});
...
app.get("/api/users/search", async (req, res) => {
    ...
    const [rows] = await pool.query(`SELECT id, nickname, avatar, role, created_at, experience, equipped_badge_key, equipped_badge_icon_url
     FROM users
     WHERE nickname LIKE ?
     ORDER BY CASE WHEN nickname = ? THEN 0 WHEN nickname LIKE ? THEN 1 ELSE 2 END, created_at DESC
     LIMIT ?`, [likeKeyword, keyword, prefixKeyword, limit]);
    const [[totalRow]] = await pool.query("SELECT COUNT(*) AS total FROM users WHERE nickname LIKE ?", [likeKeyword]);
    res.json({
...
app.get("/api/users/:id/profile", async (req, res) => {
    ...
    if (!viewer)
        return;
    const [rows] = await pool.query(`SELECT u.id, u.username, u.nickname, u.role, u.created_at, u.experience, u.charm_value, u.generosity_value, u.equipped_badge_key, u.equipped_badge_icon_url,
     ...
     WHERE u.id = ? LIMIT 1`, [req.params.id]);
    const target = rows[0];
    if (!target)
        return sendError(res, 404, "用户不存在");
    const includeSoups = req.query.includeSoups !== "false";
    const soupPromise = includeSoups ? pool.query(`SELECT ${soupSummaryColumns("s")}, NULL AS creator_avatar, u.avatar IS NOT NULL AS creator_has_avatar,
        u.experience AS creator_experience, u.equipped_badge_key AS creator_badge_key, u.equipped_badge_icon_url AS creator_badge_icon_url,
        (SELECT COUNT(*) FROM soup_likes WHERE soup_id = s.id) AS like_count,
        (SELECT COUNT(*) FROM soup_favorites WHERE soup_id = s.id) AS favorite_count,
       ...
       LIMIT 10`, [viewer.id, viewer.id, req.params.id]) : Promise.resolve([[], []]);
    const [[likeRows], [followRows], [soupRows]] = await Promise.all([
        pool.query(`SELECT COUNT(*) AS received_like_count
       FROM soup_likes sl INNER JOIN soups s ON s.id = sl.soup_id
       WHERE s.creator_id = ?`, [req.params.id]),
        pool.query(`SELECT
         ...
         EXISTS (SELECT 1 FROM user_follows WHERE follower_id = ? AND following_id = ?) AS is_following`, [req.params.id, req.params.id, viewer.id, req.params.id]),
        soupPromise
    ]);
...
app.post("/api/users/:id/follow", async (req, res) => {
    ...
    if (user.id === req.params.id)
        return sendError(res, 400, "不能关注自己");
    const [targetRows] = await pool.query("SELECT id FROM users WHERE id = ? LIMIT 1", [req.params.id]);
    if (!targetRows[0])
        return sendError(res, 404, "用户不存在");
    const [existing] = await pool.query("SELECT follower_id FROM user_follows WHERE follower_id = ? AND following_id = ? LIMIT 1", [user.id, req.params.id]);
    if (existing[0]) {
...
app.get("/api/users/:id/follows", async (req, res) => {
    ...
    const [rows] = await pool.query(`SELECT u.*,
     ...
     WHERE ${whereColumn} = ?
     ORDER BY f.created_at DESC`, [viewer.id, viewer.id, req.params.id]);
    res.json({
        users: rows.map((row) => ({
...
async function onlineSoupRoomInvite(roomId, inviteToken) {
    if (!validRoomInviteToken(roomId, inviteToken))
        return null;
    const [[room]] = await pool.query(`SELECT r.id, r.room_code, r.name, r.status, s.title AS soup_title,
     ...
     WHERE r.id = ? LIMIT 1`, [roomId]);
    if (!room || String(room.status) === "closed")
        return null;
    return {
...
async function sharedSoupCard(soupId) {
    const [rows] = await pool.query(`SELECT ${soupSummaryColumns("s")}, FALSE AS is_liked, FALSE AS is_favorited,
       (SELECT COUNT(*) FROM soup_likes WHERE soup_id = s.id) AS like_count,
       (SELECT COUNT(*) FROM soup_favorites WHERE soup_id = s.id) AS favorite_count,
     ...
     WHERE s.id = ? AND s.review_status = 'approved' GROUP BY s.id LIMIT 1`, [soupId]);
    if (!rows[0])
        return null;
...
function parseCircleMentions(value) {
    if (!value)
        return [];
    try {
        const parsed = typeof value === "string" ? JSON.parse(value) : value;
...
async function circleForMember(circleId, userId) {
    const [rows] = await pool.query(`SELECT c.id, c.name,
     ...
     WHERE c.id = ? LIMIT 1`, [userId, circleId]);
    return rows[0] ?? null;
}
function circleMessagePayload(row) {
...
async function circleMembers(circleId) {
    const [rows] = await pool.query(`SELECT u.id, u.nickname, u.avatar, u.created_at, u.role, u.experience,
       u.equipped_badge_key, u.equipped_badge_icon_url, m.joined_at
     FROM circle_members m
     INNER JOIN users u ON u.id = m.user_id
     WHERE m.circle_id = ?
     ORDER BY m.joined_at ASC`, [circleId]);
    return rows.map((row) => ({
        ...withoutPrivateUsername(toUser(row)),
...
app.get("/api/circles", async (req, res) => {
    ...
    const [rows] = await pool.query(`SELECT c.*,
     ...
     ORDER BY COALESCE(latest.created_at, c.created_at) DESC, c.created_at ASC`, [user.id, user.id]);
    const onlineCounts = new Map();
    const onlineUserIds = [...visiblyOnlineUsers];
    if (onlineUserIds.length > 0) {
        const [onlineRows] = await pool.query(`SELECT circle_id, COUNT(*) AS online_count
       FROM circle_members
       WHERE user_id IN (?)
       GROUP BY circle_id`, [onlineUserIds]);
...
app.post("/api/circles/:id/join", async (req, res) => {
    ...
    if (!user)
        return;
    const [[circle]] = await pool.query("SELECT id FROM circles WHERE id = ? LIMIT 1", [req.params.id]);
    if (!circle)
        return sendError(res, 404, "圈子不存在");
    const [result] = await pool.query("INSERT IGNORE INTO circle_members (circle_id, user_id) VALUES (?, ?)", [req.params.id, user.id]);
    if (result.affectedRows > 0) {
...
app.patch("/api/circles/:id/read", async (req, res) => {
    ...
    if (!circle)
        return sendError(res, 403, "请先加入圈子");
    await pool.query(`UPDATE circle_members
     SET last_read_sequence = COALESCE((SELECT MAX(message_sequence) FROM circle_messages WHERE circle_id = ?), 0)
     WHERE circle_id = ? AND user_id = ?`, [req.params.id, req.params.id, user.id]);
...
app.get("/api/circles/:id/mentions", async (req, res) => {
    ...
    if (!circle)
        return sendError(res, 403, "请先加入圈子");
    const [rows] = await pool.query(`SELECT m.id, m.message_sequence
     ...
     ORDER BY m.message_sequence ASC`, [req.params.id, user.id]);
    res.json({
...
app.patch("/api/circles/:id/mentions/:messageId/read", async (req, res) => {
    ...
    if (!circle)
        return sendError(res, 403, "请先加入圈子");
    await pool.query(`UPDATE circle_message_mentions
     SET read_at = COALESCE(read_at, CURRENT_TIMESTAMP)
     WHERE circle_id = ? AND message_id = ? AND user_id = ?`, [req.params.id, req.params.messageId, user.id]);
...
app.patch("/api/circles/:id/mentions/read-all", async (req, res) => {
    ...
    await pool.query(`UPDATE circle_message_mentions
     SET read_at = COALESCE(read_at, CURRENT_TIMESTAMP)
     WHERE circle_id = ? AND user_id = ? AND read_at IS NULL`, [req.params.id, user.id]);
...
app.get("/api/circles/:id/messages", async (req, res) => {
    ...
    let beforeClause = "";
    if (before) {
        const [[cursor]] = await pool.query("SELECT message_sequence FROM circle_messages WHERE id = ? AND circle_id = ? LIMIT 1", [before, req.params.id]);
        if (!cursor)
            return sendError(res, 400, "消息游标无效");
        beforeClause = "AND m.message_sequence < ?";
        params.push(cursor.message_sequence);
    }
    params.push(limit + 1);
    const [rows] = await pool.query(`SELECT m.*, u.nickname AS sender_nickname, u.experience AS sender_experience, NULL AS sender_avatar, u.avatar IS NOT NULL AS sender_has_avatar,
...
app.patch("/api/circles/:id/messages/:messageId/recall", async (req, res) => {
    ...
    if (!circle)
        return sendError(res, 403, "请先加入圈子");
    const [[message]] = await pool.query(`SELECT id, sender_id, message_type, recalled_at,
       created_at >= DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 2 MINUTE) AS within_window
     FROM circle_messages WHERE id = ? AND circle_id = ? LIMIT 1`, [req.params.messageId, req.params.id]);
    if (!message)
        return sendError(res, 404, "消息不存在");
    if (message.message_type === "gift")
        return sendError(res, 409, "礼物消息不可撤回");
    if (String(message.sender_id ?? "") !== user.id)
        return sendError(res, 403, "只能撤回自己的消息");
    if (message.recalled_at)
        return sendError(res, 409, "消息已经撤回");
    if (!bool(message.within_window))
        return sendError(res, 409, "消息发送超过2分钟，无法撤回");
    const connection = await pool.getConnection();
    try {
        await connection.beginTransaction();
        const [result] = await connection.query(`UPDATE circle_messages
       ...
       WHERE id = ? AND circle_id = ? AND sender_id = ? AND recalled_at IS NULL
         AND created_at >= DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 2 MINUTE)`, [req.params.messageId, req.params.id, user.id]);
    ...
    const [circleMembers] = await pool.query("SELECT user_id FROM circle_members WHERE circle_id = ?", [req.params.id]);
...
app.post("/api/circles/:id/messages", async (req, res) => {
    ...
    if (!parsed.success)
        return sendError(res, 400, parsed.error.issues[0]?.message ?? "消息内容不正确");
    const sticker = parsed.data.stickerId ? getSticker(parsed.data.stickerId) : null;
    if (parsed.data.stickerId && !sticker)
        return sendError(res, 400, "表情不存在或已下架");
    const roomInvite = parsed.data.roomInvite
        ...
        : null;
    if (parsed.data.roomInvite && !roomInvite)
        return sendError(res, 400, "玩汤房间邀请无效或房间已关闭");
    const soupShare = parsed.data.soupShare ? await sharedSoupCard(parsed.data.soupShare.soupId) : null;
    if (parsed.data.soupShare && !soupShare)
        return sendError(res, 400, "海龟汤不存在或暂不可分享");
    if (parsed.data.replyToMessageId) {
        const [[replyTarget]] = await pool.query("SELECT id FROM circle_messages WHERE id = ? AND circle_id = ? AND recalled_at IS NULL LIMIT 1", [parsed.data.replyToMessageId, req.params.id]);
        if (!replyTarget)
            return sendError(res, 400, "被回复的消息不存在、已撤回或不属于当前圈子");
    }
    const mentionedUserIds = [...new Set(parsed.data.mentionedUserIds ?? [])].filter((id) => id !== user.id);
    let mentions = [];
    if (mentionedUserIds.length) {
        const [mentionedMembers] = await pool.query(`SELECT u.id, u.nickname
       FROM circle_members member
       INNER JOIN users u ON u.id = member.user_id
       WHERE member.circle_id = ? AND member.user_id IN (?)`, [req.params.id, mentionedUserIds]);
        if (mentionedMembers.length !== mentionedUserIds.length)
            return sendError(res, 400, "被@的用户不在当前圈子");
    ...
    try {
        await connection.beginTransaction();
        await connection.query(`INSERT INTO circle_messages (id, circle_id, sender_id, content, message_type, sticker_id, mentions_json, reply_to_message_id)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [
            id,
            req.params.id,
            user.id,
        ...
        for (const mention of mentions) {
            await connection.query("INSERT INTO circle_message_mentions (message_id, circle_id, user_id) VALUES (?, ?, ?)", [id, req.params.id, mention.userId]);
        }
    ...
    const [[stored]] = await pool.query(`SELECT m.*, u.nickname AS sender_nickname, u.experience AS sender_experience, NULL AS sender_avatar, u.avatar IS NOT NULL AS sender_has_avatar,
       u.equipped_badge_key AS sender_badge_key, u.equipped_badge_icon_url AS sender_badge_icon_url,
       reply.id AS reply_id, reply.message_sequence AS reply_sequence,
       reply.sender_id AS reply_sender_id, reply_user.nickname AS reply_sender_nickname,
       reply.content AS reply_content, reply.message_type AS reply_message_type,
    ...
    const [circleRecipients] = await pool.query("SELECT user_id FROM circle_members WHERE circle_id = ? AND user_id <> ?", [req.params.id, user.id]);
...
app.patch("/api/admin/circles/:id/messages/:messageId/recall", async (req, res) => {
    ...
    if (!admin)
        return;
    const [[message]] = await pool.query("SELECT id, sender_id, message_type, recalled_at FROM circle_messages WHERE id = ? AND circle_id = ? LIMIT 1", [req.params.messageId, req.params.id]);
    if (!message)
        return sendError(res, 404, "消息不存在");
    if (message.message_type === "gift")
        return sendError(res, 409, "礼物消息关联真实送礼流水，不可撤回");
    if (message.recalled_at)
        return sendError(res, 409, "消息已经撤回");
    const connection = await pool.getConnection();
    try {
        ...
        const [result] = await connection.query(`UPDATE circle_messages
       SET content = '', sticker_id = NULL, mentions_json = NULL, recalled_at = CURRENT_TIMESTAMP
       WHERE id = ? AND circle_id = ? AND recalled_at IS NULL`, [req.params.messageId, req.params.id]);
        if (!result.affectedRows) {
            await connection.rollback();
            return sendError(res, 409, "消息已经撤回");
        }
    ...
    const [members] = await pool.query("SELECT user_id FROM circle_members WHERE circle_id = ?", [req.params.id]);
...
app.post("/api/admin/circles", async (req, res) => {
    ...
    if (!parsed.success)
        return sendError(res, 400, parsed.error.issues[0]?.message ?? "圈子信息不正确");
    const id = nanoid();
    const avatar = parsed.data.avatar ? await optimizeCircleAvatar(parsed.data.avatar, id) : null;
    if (parsed.data.avatar && !avatar)
        return sendError(res, 400, "头像无法处理，请使用 JPG、PNG 或 WebP");
    await pool.query("INSERT INTO circles (id, name, avatar, created_by) VALUES (?, ?, ?, ?)", [id, parsed.data.name, avatar, admin.id]);
    res.status(201).json({ id });
});
app.put("/api/admin/circles/:id", async (req, res) => {
    if (!(await requireAdmin(req, res)))
        return;
    const [[existing]] = await pool.query("SELECT * FROM circles WHERE id = ? LIMIT 1", [req.params.id]);
    if (!existing)
        return sendError(res, 404, "圈子不存在");
    const parsed = circleAdminSchema.safeParse(req.body);
    if (!parsed.success)
    ...
    await pool.query("UPDATE circles SET name = ?, avatar = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?", [parsed.data.name, avatar, req.params.id]);
    const [[updated]] = await pool.query("SELECT * FROM circles WHERE id = ? LIMIT 1", [req.params.id]);
    const circle = {
...
app.get("/api/me/received-interactions", async (req, res) => {
    ...
    if (!user)
        return;
    const [rows] = await pool.query(`SELECT s.id, s.title, s.cover_thumbnail,
       (SELECT COUNT(*) FROM soup_likes WHERE soup_id = s.id) AS like_count,
       (SELECT COUNT(*) FROM soup_favorites WHERE soup_id = s.id) AS favorite_count,
...
app.get("/api/me/soups/:id/interactions", async (req, res) => {
    ...
    if (!user)
        return;
    const [soupRows] = await pool.query("SELECT id, title FROM soups WHERE id = ? AND creator_id = ? LIMIT 1", [req.params.id, user.id]);
    if (!soupRows[0])
        return sendError(res, 404, "作品不存在或无权查看");
    const type = String(req.query.type ?? "likes");
    let rows = [];
    if (type === "likes") {
        [rows] = await pool.query(`SELECT u.id, u.nickname, NULL AS avatar, u.avatar IS NOT NULL AS has_avatar, sl.created_at
       FROM soup_likes sl INNER JOIN users u ON u.id = sl.user_id
       WHERE sl.soup_id = ? ORDER BY sl.created_at DESC`, [req.params.id]);
    }
    else if (type === "favorites") {
        [rows] = await pool.query(`SELECT u.id, u.nickname, NULL AS avatar, u.avatar IS NOT NULL AS has_avatar, sf.created_at
       FROM soup_favorites sf INNER JOIN users u ON u.id = sf.user_id
       WHERE sf.soup_id = ? ORDER BY sf.created_at DESC`, [req.params.id]);
    }
    else if (type === "evaluations") {
        [rows] = await pool.query(`SELECT u.id, u.nickname, NULL AS avatar, u.avatar IS NOT NULL AS has_avatar, e.total, e.content, e.created_at
       FROM evaluations e INNER JOIN users u ON u.id = e.reviewer_id
       WHERE e.soup_id = ? ORDER BY e.created_at DESC`, [req.params.id]);
    }
    else {
...
app.post("/api/conversations", async (req, res) => {
    ...
    if (!parsed.success || parsed.data.userId === user.id)
        return sendError(res, 400, "私信对象不正确");
    const [targetRows] = await pool.query("SELECT id FROM users WHERE id = ? LIMIT 1", [parsed.data.userId]);
    if (!targetRows[0])
        return sendError(res, 404, "用户不存在");
    const [userA, userB] = canonicalConversationUserIds(user.id, parsed.data.userId);
    const [existing] = await pool.query("SELECT id FROM conversations WHERE user_a_id = ? AND user_b_id = ? LIMIT 1", [userA, userB]);
    if (existing[0])
        return res.json({ id: String(existing[0].id) });
    const [followRows] = await pool.query("SELECT follower_id FROM user_follows WHERE follower_id = ? AND following_id = ? LIMIT 1", [user.id, parsed.data.userId]);
    if (!followRows[0])
        return sendError(res, 403, "关注对方后才能发起私信");
    const id = nanoid();
    try {
        ...
        if (error.code !== "ER_DUP_ENTRY")
            throw error;
        const [rows] = await pool.query("SELECT id FROM conversations WHERE user_a_id = ? AND user_b_id = ? LIMIT 1", [userA, userB]);
...
app.get("/api/conversations", async (req, res) => {
    ...
    if (!user)
        return;
    const [rows] = await pool.query(`SELECT c.id, c.last_message_at,
     ...
     ORDER BY c.last_message_at DESC`, [user.id, user.id, user.id, user.id]);
...
app.get("/api/messages/unread-counts", async (req, res) => {
    ...
    const [[notificationCounts], [requestCounts], [noticeCounts], [privateMessageCounts], [circleMessageCounts], [circleMentionCounts]] = await Promise.all([
       ...
       FROM notifications WHERE user_id = ?`, [...interactionTypes, ...interactionTypes, user.id]),
        pool.query(`SELECT COUNT(*) AS request_count FROM view_requests WHERE status = 'pending' ${requestWhere}`, requestParams),
        pool.query(`SELECT COUNT(*) AS notice_count
       ...
       WHERE (n.expires_at IS NULL OR n.expires_at > CURRENT_TIMESTAMP)
        ...
        pool.query(`SELECT COUNT(*) AS private_message_count
       FROM private_messages pm
       INNER JOIN conversations c ON c.id = pm.conversation_id
       WHERE (c.user_a_id = ? OR c.user_b_id = ?)
         AND pm.sender_id <> ? AND pm.read_at IS NULL`, [user.id, user.id, user.id]),
        pool.query(`SELECT COUNT(*) AS circle_message_count
       FROM circle_members member
       ...
       WHERE member.user_id = ?`, [user.id]),
        pool.query(`SELECT COUNT(*) AS circle_mention_count
    ...
    if (unreadCountsCache.size > 1000)
        unreadCountsCache.delete(unreadCountsCache.keys().next().value);
    res.json(payload);
});
async function conversationForUser(conversationId, userId) {
    const [rows] = await pool.query(`SELECT c.*, u.id AS other_id, u.nickname AS other_nickname, NULL AS other_avatar, u.avatar IS NOT NULL AS other_has_avatar,
     ...
     FROM conversations c
     INNER JOIN users u ON u.id = IF(c.user_a_id = ?, c.user_b_id, c.user_a_id)
     WHERE c.id = ? AND (c.user_a_id = ? OR c.user_b_id = ?) LIMIT 1`, [userId, userId, conversationId, userId, userId]);
    return rows[0] ?? null;
}
app.get("/api/conversations/:id/messages", async (req, res) => {
    ...
    if (!conversation)
        return sendError(res, 404, "会话不存在");
    const [readResult] = await pool.query("UPDATE private_messages SET read_at = CURRENT_TIMESTAMP WHERE conversation_id = ? AND sender_id <> ? AND read_at IS NULL", [req.params.id, user.id]);
    if (readResult.affectedRows > 0)
        emitUnreadChanged(user.id, "private_message_read");
    ...
    let beforeClause = "";
    if (before) {
        const [[cursor]] = await pool.query("SELECT created_at, id FROM private_messages WHERE id = ? AND conversation_id = ? LIMIT 1", [before, req.params.id]);
        if (!cursor)
            return sendError(res, 400, "消息游标无效");
    ...
    params.push(limit + 1);
    const [rows] = await pool.query(`SELECT id, sender_id, content, message_type, sticker_id, read_at, created_at, recalled_at
...
app.patch("/api/conversations/:id/read", async (req, res) => {
    ...
    if (!conversation)
        return sendError(res, 404, "会话不存在");
    const [result] = await pool.query("UPDATE private_messages SET read_at = CURRENT_TIMESTAMP WHERE conversation_id = ? AND sender_id <> ? AND read_at IS NULL", [req.params.id, user.id]);
    if (result.affectedRows > 0)
        emitUnreadChanged(user.id, "private_message_read");
...
app.patch("/api/conversations/:id/messages/:messageId/recall", async (req, res) => {
    ...
    const [[message]] = await pool.query(`SELECT id, sender_id, message_type, recalled_at,
       created_at >= DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 2 MINUTE) AS within_window
     FROM private_messages WHERE id = ? AND conversation_id = ? LIMIT 1`, [req.params.messageId, req.params.id]);
    if (!message)
        return sendError(res, 404, "消息不存在");
    if (message.message_type === "gift")
        return sendError(res, 409, "礼物消息不可撤回");
    ...
    if (!bool(message.within_window))
        return sendError(res, 409, "消息发送超过2分钟，无法撤回");
    const [result] = await pool.query(`UPDATE private_messages
...
app.post("/api/conversations/:id/messages", async (req, res) => {
    ...
    try {
        await connection.beginTransaction();
        await connection.query("INSERT INTO private_messages (id, conversation_id, sender_id, content, message_type, sticker_id) VALUES (?, ?, ?, ?, ?, ?)", [id, req.params.id, user.id, content, messageType, sticker?.id ?? null]);
...
app.post("/api/me/badge-unlocks/sync", async (req, res) => {
    ...
    if (!user)
        return;
    queueLoginDayRecord(user.id);
    const [userRows] = await pool.query("SELECT badges_initialized FROM users WHERE id = ? LIMIT 1", [user.id]);
...
app.get("/api/me/legendary-badges", async (req, res) => {
    ...
    if (!user)
        return;
    const [rows] = await pool.query(`SELECT lb.id, lb.name, lb.description, lb.requirement, lb.icon_url, lb.achievement_points,
       lb.badge_type, lb.tier, lb.activity_conditions, ubu.unlocked_at
     FROM legendary_badges lb
     INNER JOIN user_badge_unlocks ubu ON ubu.badge_key = CONCAT('legendary:', lb.id)
     WHERE ubu.user_id = ?
     ORDER BY ubu.unlocked_at DESC`, [user.id]);
    res.json({
        badges: rows.map((row) => ({
            id: String(row.id),
            key: `legendary:${row.id}`,
...
app.get("/api/me/badge-collection", async (req, res) => {
    ...
    if (!user)
        return;
    const [unlockRows] = await pool.query("SELECT badge_key FROM user_badge_unlocks WHERE user_id = ? ORDER BY unlocked_at DESC", [user.id]);
    const [legendaryRows] = await pool.query(`SELECT lb.id, lb.name, lb.description, lb.requirement, lb.icon_url, lb.achievement_points,
       lb.badge_type, lb.tier, lb.activity_conditions, ubu.unlocked_at
     FROM legendary_badges lb
     INNER JOIN user_badge_unlocks ubu ON ubu.badge_key = CONCAT('legendary:', lb.id)
     WHERE ubu.user_id = ?
     ORDER BY ubu.unlocked_at DESC`, [user.id]);
    const [[freshUser]] = await pool.query("SELECT equipped_badge_key, equipped_badge_icon_url FROM users WHERE id = ? LIMIT 1", [user.id]);
    res.json({
...
app.patch("/api/me/equipped-badge", async (req, res) => {
    ...
    if (!parsed.success)
        return sendError(res, 400, "徽章参数不正确");
    if (parsed.data.badgeKey === null) {
        await pool.query("UPDATE users SET equipped_badge_key = NULL, equipped_badge_icon_url = NULL WHERE id = ?", [user.id]);
        return res.json({ equippedBadge: null });
    }
    const iconUrl = await ownedBadgeIconUrl(user.id, parsed.data.badgeKey);
    if (!iconUrl)
        return sendError(res, 403, "只能装配自己已获得的徽章");
    await pool.query("UPDATE users SET equipped_badge_key = ?, equipped_badge_icon_url = ? WHERE id = ?", [parsed.data.badgeKey, iconUrl, user.id]);
...
function rankingCutoff(period) {
    if (period === "all")
        return null;
    return new Date(Date.now() - (period === "7d" ? 7 : 30) * 24 * 60 * 60 * 1000);
}
function heatFromParts(score, views, likes, favorites, evaluations) {
...
app.get("/api/rankings", async (req, res) => {
    ...
    if (!cached || cached.expiresAt <= Date.now()) {
        const [hotSoupRows] = await pool.query(`SELECT s.id, s.title, s.author, s.creator_id, s.view_count,
         s.created_at,
       ...
       ORDER BY s.created_at ASC`, [cutoff ?? new Date(0), cutoff ?? new Date(0), cutoff ?? new Date(0), cutoff ?? new Date(0), cutoff ?? new Date(0)]);
        const [achievementRows] = await pool.query(`SELECT u.id, u.nickname, u.created_at, u.avatar IS NOT NULL AS has_avatar,
       ...
       ORDER BY u.created_at ASC, ubu.unlocked_at ASC`);
        const [levelRows] = await pool.query(`SELECT u.id, u.nickname, u.experience, u.created_at, u.avatar IS NOT NULL AS has_avatar,
       ...
       WHERE u.role IN ('user', 'vip', 'backoffice_admin')
       ORDER BY u.created_at ASC, u.id ASC`, [cutoff ?? new Date(0), cutoff ?? new Date(0), cutoff ?? new Date(0), cutoff ?? new Date(0), cutoff ?? new Date(0)]);
        const [charmRows] = await pool.query(`SELECT u.id, u.nickname, u.charm_value, u.created_at, u.avatar IS NOT NULL AS has_avatar,
       ...
       ORDER BY u.created_at ASC, u.id ASC`, [cutoff ?? new Date(0), cutoff ?? new Date(0)]);
        const [generosityRows] = await pool.query(`SELECT u.id, u.nickname, u.generosity_value, u.created_at, u.avatar IS NOT NULL AS has_avatar,
       ...
       ORDER BY u.created_at ASC, u.id ASC`, [cutoff ?? new Date(0), cutoff ?? new Date(0)]);
        ...
        const hotSoups = hotSoupRows
            .map((row) => {
            const currentHeat = heatFromParts(Number(row.comprehensive_score ?? 0), Number(row.view_count ?? 0), Number(row.like_count ?? 0), Number(row.favorite_count ?? 0), Number(row.evaluation_count ?? 0));
            const periodHeat = heatFromParts(Number(row.period_comprehensive_score ?? 0), Number(row.period_view_count ?? 0), Number(row.period_like_count ?? 0), Number(row.period_favorite_count ?? 0), Number(row.period_evaluation_count ?? 0));
            return {
                id: String(row.id),
                title: String(row.title),
                author: String(row.author),
    ...
    const toPublicHotSoup = ({ creatorId: _creatorId, ...item }) => item;
    res.json({
...
app.get("/api/me/favorites", async (req, res) => {
    ...
    const rawOffset = Number(req.query.offset ?? 0);
    const offset = Number.isFinite(rawOffset) ? Math.max(0, Math.floor(rawOffset)) : 0;
    const [[totalRow]] = await pool.query(`SELECT COUNT(*) AS total FROM soup_favorites f INNER JOIN soups s ON s.id = f.soup_id WHERE f.user_id = ? AND s.review_status = 'approved'`, [user.id]);
    const [rows] = await pool.query(`
    SELECT ${soupSummaryColumns("s")}, NULL AS creator_avatar, u.avatar IS NOT NULL AS creator_has_avatar,
      u.experience AS creator_experience, u.equipped_badge_key AS creator_badge_key, u.equipped_badge_icon_url AS creator_badge_icon_url,
      (SELECT COUNT(*) FROM soup_likes WHERE soup_id = s.id) AS like_count,
...
let homeFeaturedSoupCache = null;
...
app.get("/api/soups", async (req, res) => {
    ...
    if (req.query.original === "yes")
        where.push("s.is_original = TRUE");
    if (req.query.original === "no")
        where.push("s.is_original = FALSE");
    if (["简单", "普通", "困难", "地狱"].includes(String(req.query.difficulty ?? ""))) {
        where.push("s.difficulty = ?");
        params.push(String(req.query.difficulty));
    }
    if (req.query.bottomPublic === "surface")
        where.push("s.is_surface_public = TRUE");
    if (req.query.bottomPublic === "bottom")
        where.push("s.is_bottom_public = TRUE");
    ...
    const useHomeFeaturedOrder = homeCategory === "recommended" &&
    ...
    * (
    ...
    if (having.length === 0) {
        ...
        const [pageRows] = await pool.query(`${summarySelect(true)}
       FROM (
       ...
       ORDER BY ${orderClause}`, [
        ...
        const [filteredRows] = await pool.query(`${summarySelect()}
       FROM soups s
       ${scoringEvaluationJoin("e", "s")}
       LEFT JOIN users u ON u.id = s.creator_id
       ${where.length ? `WHERE ${where.join(" AND ")}` : ""}
       ...
       ORDER BY ${orderClause}
       LIMIT ${limit + 1} OFFSET ${offset}`, [...(user ? [user.id, user.id] : []), ...params, ...orderParams]);
        rows = filteredRows;
    }
    const hasMore = rows.length > limit;
    if (hasMore)
    ...
    if (req.query.includeTotal !== "0") {
        const [[totalRow]] = await pool.query(`SELECT COUNT(*) AS total FROM (
        SELECT s.id
        FROM soups s
        ${scoringEvaluationJoin("e", "s")}
        ${where.length ? `WHERE ${where.join(" AND ")}` : ""}
...
app.post("/api/soups", async (req, res) => {
    ...
    if (!parsed.success)
        return sendError(res, 400, "请完整填写海龟汤信息");
    ...
    if (idempotencyKey) {
        const [[existingSoup]] = await pool.query("SELECT id, creator_id, review_status FROM soups WHERE id = ? LIMIT 1", [id]);
        if (existingSoup) {
            if (String(existingSoup.creator_id) !== user.id)
                return sendError(res, 409, "防重复提交标识冲突");
    ...
    if (duplicate)
        return sendError(res, 409, "该海龟汤在平台上高度重复");
    ...
    let review;
    if (isSuperAdminRole(user.role)) {
        review = { decision: "approved", reason: null };
    }
    else {
        try {
            ...
            if (error instanceof SoupReviewUnavailableError)
    ...
    if (soup.coverImage && !optimizedCover)
        return sendError(res, 400, "封面图片无法处理");
    const connection = await pool.getConnection();
    try {
        await connection.beginTransaction();
        if (!hasUnlimitedPublishing) {
            await connection.query("INSERT IGNORE INTO soup_publish_daily_usage (user_id, usage_date) VALUES (?, ?)", [user.id, usage.date]);
            const [quotaResult] = await connection.query(`UPDATE soup_publish_daily_usage SET published_count = published_count + 1
         WHERE user_id = ? AND usage_date = ? AND published_count < ? AND auto_reject_count < ?`, [user.id, usage.date, SOUP_DAILY_PUBLISH_LIMIT, SOUP_DAILY_AUTO_REJECT_LIMIT]);
            if (quotaResult.affectedRows !== 1)
                throw new Error("SOUP_DAILY_QUOTA");
        }
        await connection.query(`INSERT INTO soups
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [id, soup.title, author, soup.type, soup.difficulty, soup.summary, optimizedCover?.full ?? null, optimizedCover?.thumbnail ?? null, soup.isOriginal, soup.isSensitive,
...
app.get("/api/soups/:id", async (req, res) => {
    ...
    if (Date.now() - lastView > 60_000) {
        await pool.query("INSERT INTO soup_views (id, soup_id, user_identifier) VALUES (?, ?, ?)", [nanoid(), req.params.id, identifier]);
        await pool.query("UPDATE soups SET view_count = view_count + 1 WHERE id = ?", [req.params.id]);
        if (user)
            recordUserBehavior("view_soup");
    }
    ...
    LIMIT 1
    `, [req.params.id]),
        pool.query(`SELECT e.*, NULL AS reviewer_avatar, reviewer.avatar IS NOT NULL AS reviewer_has_avatar,
    ...
    const [requestRows, favoriteRows, likeRows] = user
        ? await Promise.all([
            !full ? pool.query("SELECT id FROM view_requests WHERE soup_id = ? AND requester_id = ? AND status = 'pending' LIMIT 1", [req.params.id, user.id]).then(([rows]) => rows) : Promise.resolve([]),
            pool.query("SELECT id FROM soup_favorites WHERE soup_id = ? AND user_id = ? LIMIT 1", [req.params.id, user.id]).then(([rows]) => rows),
            pool.query("SELECT id FROM soup_likes WHERE soup_id = ? AND user_id = ? LIMIT 1", [req.params.id, user.id]).then(([rows]) => rows)
        ])
        : [[], [], []];
    ...
    res.json({
        soup: {
            ...
            aiPrompt: canEdit ? soup.ai_prompt || null : null,
            keyFacts: canEdit ? safeParseJson(soup.key_facts) : null,
            keyFactsCustomized: canEdit && soup.key_facts_customized === 1,
            canViewFull: full,
            canEdit,
...
app.post("/api/soups/:id/like", async (req, res) => {
    ...
    if (!soup)
        return sendError(res, 404, "海龟汤不存在");
    if (String(soup.review_status ?? "approved") !== "approved")
        return sendError(res, 409, "审核中的海龟汤暂不可点赞");
    if (!canSeeSoupSurface(soup, user))
        return sendError(res, 403, "没有查看权限");
    const connection = await pool.getConnection();
    let isLiked = false;
    ...
    try {
        await connection.beginTransaction();
        const [rows] = await connection.query("SELECT id FROM soup_likes WHERE soup_id = ? AND user_id = ? LIMIT 1 FOR UPDATE", [req.params.id, user.id]);
        if (rows.length > 0) {
            ...
            if (Boolean(soup.is_original)) {
                await connection.query("INSERT IGNORE INTO soup_like_history (soup_id, actor_id, creator_id) VALUES (?, ?, ?)", [req.params.id, user.id, soup.creator_id]);
            }
            const [rewardHistoryResult] = await connection.query("INSERT IGNORE INTO shell_like_reward_history (soup_id, user_id) VALUES (?, ?)", [req.params.id, user.id]);
            ...
            if (String(soup.creator_id) !== user.id) {
                await awardShellTask(String(soup.creator_id), "receive_soup_like", `receive-like:${user.id}:${req.params.id}`, { relatedType: "soup", relatedId: req.params.id, connection });
            }
        }
        const [[countRow]] = await connection.query("SELECT COUNT(*) AS cnt FROM soup_likes WHERE soup_id = ?", [req.params.id]);
    ...
    if (!isLiked)
        return res.json({ isLiked: false, likeCount });
    recordUserBehavior("like_soup");
    res.status(201).json({ isLiked: true, likeCount });
    if (String(soup.creator_id) !== user.id) {
        void notify(String(soup.creator_id), "soup_like", "收到新的点赞", `${user.nickname} 点赞了你的海龟汤《${soup.title}》`, req.params.id, user.id).catch((error) => console.error("Failed to create soup-like notification:", error));
    }
...
app.get("/api/me/likes", async (req, res) => {
    ...
    const rawOffset = Number(req.query.offset ?? 0);
    const offset = Number.isFinite(rawOffset) ? Math.max(0, Math.floor(rawOffset)) : 0;
    const [[totalRow]] = await pool.query(`SELECT COUNT(*) AS total FROM soup_likes lk INNER JOIN soups s ON s.id = lk.soup_id WHERE lk.user_id = ? AND s.review_status = 'approved'`, [user.id]);
    const [rows] = await pool.query(`
...
app.post("/api/soups/:id/favorite", async (req, res) => {
    ...
    if (!soup)
        return sendError(res, 404, "海龟汤不存在");
    if (String(soup.review_status ?? "approved") !== "approved")
        return sendError(res, 409, "审核中的海龟汤暂不可收藏");
    if (!canSeeSoupSurface(soup, user))
        return sendError(res, 403, "没有查看权限");
    const connection = await pool.getConnection();
    ...
    try {
        await connection.beginTransaction();
        const [rows] = await connection.query("SELECT id FROM soup_favorites WHERE soup_id = ? AND user_id = ? LIMIT 1 FOR UPDATE", [req.params.id, user.id]);
        if (rows.length > 0) {
            ...
            if (Boolean(soup.is_original)) {
                await connection.query("INSERT IGNORE INTO soup_favorite_history (soup_id, actor_id, creator_id) VALUES (?, ?, ?)", [req.params.id, user.id, soup.creator_id]);
            }
            const taskDate = beijingTaskDate();
            ...
            if (String(soup.creator_id) !== user.id) {
                await awardShellTask(String(soup.creator_id), "receive_soup_favorite", `receive-favorite:${user.id}:${req.params.id}`, { relatedType: "soup", relatedId: req.params.id, connection });
            }
        }
        const [[countRow]] = await connection.query("SELECT COUNT(*) AS cnt FROM soup_favorites WHERE soup_id = ?", [req.params.id]);
    ...
    if (!isFavorited)
        return res.json({ isFavorited: false, favoriteCount });
    recordUserBehavior("favorite_soup");
    res.status(201).json({ isFavorited: true, favoriteCount });
    if (String(soup.creator_id) !== user.id) {
        void notify(String(soup.creator_id), "soup_favorite", "收到新的收藏", `${user.nickname} 收藏了你的海龟汤《${soup.title}》`, req.params.id, user.id).catch((error) => console.error("Failed to create soup-favorite notification:", error));
    }
    queueSystemBadgeSync([user.id, String(soup.creator_id)]);
});
app.put("/api/soups/:id", async (req, res) => {
    ...
    if (!soup)
        return sendError(res, 404, "海龟汤不存在");
    if (!isSuperAdminRole(user.role) && user.id !== soup.creator_id)
        return sendError(res, 403, "没有编辑权限");
    const [[creator]] = await pool.query("SELECT role FROM users WHERE id = ? LIMIT 1", [soup.creator_id]);
    ...
    const parsed = soupSchema.safeParse(normalizeExistingSoupCover(req.body, req.params.id, existingCoverUrl));
    if (!parsed.success)
        return sendError(res, 400, "请完整填写海龟汤信息");
    ...
    if (!reviewContentChanged) {
        review = {
            decision: String(soup.review_status),
    ...
    if (next.coverImage && !existingCoverSelected && !optimizedCover)
        return sendError(res, 400, "封面图片无法处理");
    ...
    try {
        await connection.beginTransaction();
        await connection.query(`UPDATE soups
...
app.post("/api/soups/:id/reanalyze-keyfacts", async (req, res) => {
    ...
    if (!isSuperAdminRole(user.role) && user.id !== soup.creator_id)
        return sendError(res, 403, "没有编辑权限");
    const [[creator]] = await pool.query("SELECT role FROM users WHERE id = ? LIMIT 1", [soup.creator_id]);
    if (!canEnableAiGameRole(creator?.role))
        return sendError(res, 403, "该上传者无权配置 AI 玩汤");
...
app.delete("/api/soups/:id", async (req, res) => {
    ...
    if (!soup)
        return sendError(res, 404, "海龟汤不存在");
    if (!isSuperAdminRole(user.role) && user.id !== soup.creator_id)
        return sendError(res, 403, "没有删除权限");
...
app.post("/api/admin/soups/:id/review", async (req, res) => {
    ...
    if (!parsed.success)
        return sendError(res, 400, "审核参数不正确");
    const connection = await pool.getConnection();
    let soup;
    try {
        await connection.beginTransaction();
        const [rows] = await connection.query("SELECT id, title, creator_id, is_surface_public, review_status, review_version FROM soups WHERE id = ? LIMIT 1 FOR UPDATE", [req.params.id]);
        soup = rows[0];
        if (!soup) {
            await connection.rollback();
            return sendError(res, 404, "海龟汤不存在");
        ...
        const [result] = await connection.query(`UPDATE soups SET review_status = ?, review_reason = ?, reviewed_at = NOW(), reviewed_by = ?
       WHERE id = ? AND review_status = 'pending' AND review_version = ?`, [parsed.data.decision, parsed.data.reason || (parsed.data.decision === "approved" ? null : "内容未通过人工审核"), admin.id, req.params.id, parsed.data.reviewVersion]);
    ...
    await notify(String(soup.creator_id), "soup_review", parsed.data.decision === "approved" ? "海龟汤审核通过" : "海龟汤审核未通过", `《${soup.title}》${parsed.data.decision === "approved" ? "已通过管理员审核并公开" : "未通过管理员审核，可修改后重新提交"}`, req.params.id, admin.id);
    res.json({ ok: true });
});
app.post("/api/soups/:id/evaluations", async (req, res) => {
    ...
    if (!soup)
        return sendError(res, 404, "海龟汤不存在");
    if (String(soup.review_status ?? "approved") !== "approved")
        return sendError(res, 409, "审核中的海龟汤暂不可评价");
    if (!canSeeSoupSurface(soup, user))
        return sendError(res, 403, "不能评价未公开内容");
    if (!(await canViewFull(soup, user)))
        return sendError(res, 403, "获得汤底查看权限后才能评价");
    const parsed = evaluationSchema.safeParse(req.body);
    if (!parsed.success)
        return sendError(res, 400, "评分必须在 1-5 之间，步长 0.5");
    ...
    try {
        await connection.beginTransaction();
        const [insertResult] = await connection.query(`INSERT IGNORE INTO evaluations
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
        ...
        if (!created) {
            await connection.query(`UPDATE evaluations
         ...
         WHERE soup_id = ? AND reviewer_id = ?`, [
            ...
            const [[savedEvaluation]] = await connection.query("SELECT id FROM evaluations WHERE soup_id = ? AND reviewer_id = ? LIMIT 1", [req.params.id, user.id]);
            savedId = String(savedEvaluation.id);
        }
        if (data.content?.trim()) {
            await connection.query("INSERT IGNORE INTO evaluation_comment_history (soup_id, reviewer_id, creator_id, is_original) VALUES (?, ?, ?, ?)", [req.params.id, user.id, soup.creator_id, Boolean(soup.is_original)]);
        }
        if (created) {
        ...
        if (data.content?.trim() && String(soup.creator_id) !== user.id) {
            await awardShellTask(String(soup.creator_id), "receive_soup_evaluation", `receive-evaluation:${user.id}:${req.params.id}`, { relatedType: "soup", relatedId: req.params.id, connection });
        }
        await connection.commit();
    }
    catch (error) {
    ...
    if (created && String(soup.creator_id) !== user.id) {
        await notify(String(soup.creator_id), "soup_evaluation", "收到新的评价", `${user.nickname} 评价了你的海龟汤《${soup.title}》，评分 ${data.total} 分`, req.params.id, user.id);
    }
    queueSystemBadgeSync([user.id, String(soup.creator_id)]);
...
app.delete("/api/evaluations/:id", async (req, res) => {
    ...
    if (!user)
        return;
    const [rows] = await pool.query("SELECT * FROM evaluations WHERE id = ? LIMIT 1", [
        req.params.id
    ]);
    const evaluation = rows[0];
    if (!evaluation)
        return sendError(res, 404, "评价不存在");
    if (!isBackofficeAdminRole(user.role) && user.id !== evaluation.reviewer_id)
        return sendError(res, 403, "没有删除权限");
...
app.post("/api/soups/:id/access-requests", async (req, res) => {
    ...
    if (!soup)
        return sendError(res, 404, "海龟汤不存在");
    if (String(soup.review_status ?? "approved") !== "approved")
        return sendError(res, 409, "审核中的海龟汤暂不可申请查看");
    if (!canSeeSoupSurface(soup, user))
        return sendError(res, 403, "不能申请未公开汤面");
    if (await canViewFull(soup, user))
        return sendError(res, 409, "已经拥有查看权限");
    const [pending] = await pool.query("SELECT id FROM view_requests WHERE soup_id = ? AND requester_id = ? AND status = 'pending' LIMIT 1", [req.params.id, user.id]);
    if (pending.length)
        return sendError(res, 409, "已有待处理申请");
    const id = nanoid();
    await pool.query("INSERT INTO view_requests (id, soup_id, requester_id, requester_name, owner_id) VALUES (?, ?, ?, ?, ?)", [id, req.params.id, user.id, user.nickname, soup.creator_id]);
    await recordAdminModuleEvent("approvals");
    const recipients = new Set([soup.creator_id, ...(await adminIds())]);
    await Promise.all([...recipients].map((recipient) => notify(recipient, "view_request", "新的查看申请", `${user.nickname} 申请查看《${soup.title}》的汤底和主持人手册`, id)));
...
app.get("/api/access-requests", async (req, res) => {
    ...
    const [[totalRow]] = await pool.query(`SELECT COUNT(*) AS total FROM view_requests vr ${where}`, params);
    res.json({
        total: Number(totalRow.total ?? 0),
...
app.post("/api/access-requests/:id/decision", async (req, res) => {
    ...
    if (!parsed.success)
        return sendError(res, 400, "审批结果不正确");
    const [rows] = await pool.query("SELECT * FROM view_requests WHERE id = ? LIMIT 1", [
        req.params.id
    ]);
    const request = rows[0];
    if (!request)
        return sendError(res, 404, "申请不存在");
    if (request.status !== "pending")
        return sendError(res, 409, "申请已处理");
    if (!isBackofficeAdminRole(user.role) && user.id !== request.owner_id)
        return sendError(res, 403, "没有审批权限");
    await pool.query("UPDATE view_requests SET status = ?, handled_at = NOW(), handled_by = ? WHERE id = ?", [parsed.data.decision, user.id, req.params.id]);
    if (parsed.data.decision === "approved") {
        await pool.query("INSERT IGNORE INTO soup_access_grants (id, soup_id, user_id, granted_by) VALUES (?, ?, ?, ?)", [nanoid(), request.soup_id, request.requester_id, user.id]);
    }
    await notify(request.requester_id, "view_request_result", parsed.data.decision === "approved" ? "查看申请已通过" : "查看申请已拒绝", `你对该海龟汤完整内容的查看申请已${parsed.data.decision === "approved" ? "通过" : "拒绝"}`, request.soup_id);
    const requestRecipients = new Set([String(request.owner_id), ...(await adminIds())]);
    for (const recipient of requestRecipients)
        emitUnreadChanged(recipient, "view_request_decision");
    res.json({ ok: true });
});
app.get("/api/admin/excellent-author-applications", async (req, res) => {
    ...
    const [rows] = await pool.query(`SELECT id FROM excellent_author_applications
     ORDER BY created_at DESC
     LIMIT ? OFFSET ?`, [limit, offset]);
    const [[totalRow]] = await pool.query("SELECT COUNT(*) AS total FROM excellent_author_applications");
...
app.get("/api/admin/excellent-author-applications/:id", async (req, res) => {
    ...
    if (!application)
        return sendError(res, 404, "优秀作者认证申请不存在");
...
app.post("/api/admin/excellent-author-applications/:id/decision", async (req, res) => {
    ...
    const connection = await pool.getConnection();
    let excellentBadgeGrant = null;
    try {
        await connection.beginTransaction();
        const [updated] = await connection.query(`UPDATE excellent_author_applications
       SET status = ?, handled_at = NOW(), handled_by = ?
       WHERE id = ? AND status = 'pending'`, [parsed.data.decision, admin.id, req.params.id]);
    ...
    if (parsed.data.decision === "approved") {
        if (excellentBadgeGrant?.notificationCreated)
            emitUnreadChanged(application.applicantId, "badge_unlock");
        if ((excellentBadgeGrant?.shellReward ?? 0) > 0)
            queueSystemBadgeSync([application.applicantId]);
        if (!excellentBadgeGrant?.granted) {
            await notify(application.applicantId, "excellent_author_result", "优秀作者认证已通过", "恭喜你通过优秀作者认证，优秀作者徽章已在你的收藏中", application.id, admin.id);
        }
    }
    else {
        await notify(application.applicantId, "excellent_author_result", "优秀作者认证未通过", "你的优秀作者认证申请已被驳回，可调整作品后重新申请", application.id, admin.id);
    }
    res.json({ ok: true });
});
app.get("/api/ranking-rewards/:settlementId", async (req, res) => {
    ...
    if (!user)
        return;
    const [rows] = await pool.query(`SELECT settlements.id AS settlement_id, settlements.period_type,
     ...
     ORDER BY FIELD(grants.board_type, 'achievement', 'level', 'collection', 'charm', 'generosity', 'draws')`, [req.params.settlementId, user.id]);
    if (!rows[0])
        return sendError(res, 404, "排行榜奖励结算不存在");
    const first = rows[0];
    res.json({
        settlement: {
            ...
            grants: rows.map((row) => {
                const board = String(row.board_type);
...
app.get("/api/notices", async (req, res) => {
    ...
    const requestedOffset = Number(req.query.offset ?? 0);
    const offset = Math.max(0, Number.isFinite(requestedOffset) ? requestedOffset : 0);
    const [rows] = await pool.query(`SELECT n.id, n.title, n.author, n.published_at, n.updated_at, n.expires_at, n.valid_duration_minutes,
       EXISTS (
    ...
    const [[totalRow]] = await pool.query("SELECT COUNT(*) AS total FROM admin_notices WHERE expires_at IS NULL OR expires_at > CURRENT_TIMESTAMP");
    res.json({
        total: Number(totalRow.total ?? 0),
...
app.get("/api/notices/popup", async (req, res) => {
    ...
    if (!user)
        return;
    const [rows] = await pool.query(`SELECT n.id, n.title, n.content, n.published_at
     FROM admin_notices n
     WHERE (n.expires_at IS NULL OR n.expires_at > CURRENT_TIMESTAMP)
       AND NOT EXISTS (
         SELECT 1 FROM admin_notice_reads nr
...
app.patch("/api/notices/:id/read", async (req, res) => {
    ...
    if (!user)
        return;
    const [result] = await pool.query(`INSERT IGNORE INTO admin_notice_reads (notice_id, user_id)
     SELECT id, ? FROM admin_notices
     WHERE id = ?`, [user.id, req.params.id]);
    if (result.affectedRows > 0)
        emitUnreadChanged(user.id, "notice_read");
    res.json({ ok: true });
});
app.get("/api/notices/:id", async (req, res) => {
    ...
    if (!user)
        return;
    const [rows] = await pool.query(`SELECT id, title, author, content, published_at, updated_at, expires_at, valid_duration_minutes
     FROM admin_notices
     WHERE id = ? AND (expires_at IS NULL OR expires_at > CURRENT_TIMESTAMP)
     LIMIT 1`, [req.params.id]);
    if (!rows[0])
        return sendError(res, 404, "通知不存在");
    const [noticeReadResult] = await pool.query("INSERT IGNORE INTO admin_notice_reads (notice_id, user_id) VALUES (?, ?)", [req.params.id, user.id]);
    if (noticeReadResult.affectedRows > 0)
        emitUnreadChanged(user.id, "notice_read");
...
const dashboardRanges = { "7d": 7, "15d": 15, "30d": 30, "90d": 90 };
...
async function loadDashboardMetric(table, now, todayStart, weekStart) {
    ...
    const [[row]] = await pool.query(`SELECT
     ...
     FROM ${table}`, [todayStart, now, yesterdayStart, yesterdaySameTime, weekStart, now, previousWeekStart, previousWeekSameTime]);
...
app.get("/api/admin/dashboard", async (req, res) => {
    if (!(await requireBackofficeAdmin(req, res)))
        return;
    const parsedRange = z.enum(["7d", "15d", "30d", "90d"]).safeParse(req.query.range ?? "30d");
    if (!parsedRange.success)
        return sendError(res, 400, "统计时间范围不正确");
    const range = parsedRange.data;
    if (req.query.refresh === "1")
        await flushUserBehaviorAnalytics();
    ...
    const [userMetric, soupMetric, evaluationMetric, [trendRows], [activityRows], [activityDailyRows], [typeRows], [soupStateRows], [behaviorRows], [evaluationRows]] = await Promise.all([
       ...
       GROUP BY day_key ORDER BY day_key`, [rangeStart, now, rangeStart, now, rangeStart, now]),
        pool.query(`SELECT
       ...
       FROM user_login_days`, [todayKey, last7StartKey, todayKey]),
        pool.query(`SELECT DATE_FORMAT(login_date, '%Y-%m-%d') AS day_key, COUNT(DISTINCT user_id) AS active_count
       FROM user_login_days WHERE login_date >= ? AND login_date <= ?
       GROUP BY login_date ORDER BY login_date`, [rangeStartKey, todayKey]),
        pool.query("SELECT type AS name, COUNT(*) AS count FROM soups GROUP BY type ORDER BY count DESC, type ASC"),
        pool.query(`SELECT
       ...
       FROM soups`),
        pool.query(`SELECT DATE_FORMAT(stat_date, '%Y-%m-%d') AS day_key, behavior_type,
        historical_count + tracked_count AS event_count
       FROM user_behavior_daily_stats
       WHERE stat_date >= ? AND stat_date <= ?
       ORDER BY stat_date, behavior_type`, [rangeStartKey, todayKey]),
        pool.query(`SELECT
    ...
    const behaviorByDate = new Map();
    const behaviorTotals = new Map();
...
app.get("/api/admin/badges/users", async (req, res) => {
    ...
    const [rows] = await pool.query(`SELECT u.id, u.username, u.nickname, u.avatar,
...
app.get("/api/admin/badges/users/:id", async (req, res) => {
    if (!(await requireAdmin(req, res)))
        return;
    const [userRows] = await pool.query("SELECT id, username, nickname, avatar FROM users WHERE id = ? LIMIT 1", [req.params.id]);
    if (userRows.length === 0)
        return sendError(res, 404, "用户不存在");
    const [badgeRows] = await pool.query("SELECT badge_key FROM user_badge_unlocks WHERE user_id = ? ORDER BY unlocked_at DESC", [req.params.id]);
...
app.patch("/api/admin/badges/:id/activity-conditions", async (req, res) => {
    ...
    if (!parsed.success)
        return sendError(res, 400, parsed.error.issues[0]?.message ?? "活动条件不正确");
    const [[badge]] = await pool.query("SELECT id, badge_type FROM legendary_badges WHERE id = ? LIMIT 1", [req.params.id]);
    if (!badge)
        return sendError(res, 404, "徽章不存在");
    if (String(badge.badge_type) !== "activity")
        return sendError(res, 403, "只有活动徽章可以设置发放条件");
    await pool.query("UPDATE legendary_badges SET activity_conditions = ? WHERE id = ?", [parsed.data.conditions.length > 0 ? JSON.stringify(parsed.data.conditions) : null, req.params.id]);
    if (parsed.data.conditions.length > 0) {
        const [users] = await pool.query("SELECT id FROM users WHERE role IN ('user', 'vip')");
...
app.get("/api/admin/badges/:id/owners", async (req, res) => {
    if (!(await requireAdmin(req, res)))
        return;
    const [badgeRows] = await pool.query("SELECT id, name FROM legendary_badges WHERE id = ? LIMIT 1", [req.params.id]);
    if (badgeRows.length === 0)
        return sendError(res, 404, "传说徽章不存在");
    const [rows] = await pool.query(`SELECT u.id, u.username, u.nickname, u.avatar
     FROM user_badge_unlocks ubu
     INNER JOIN users u ON u.id = ubu.user_id
     WHERE ubu.badge_key = ?
     ORDER BY ubu.unlocked_at DESC`, [`legendary:${req.params.id}`]);
    res.json({
...
app.post("/api/admin/badges/users/:userId/legendary/:badgeId", async (req, res) => {
    ...
    if (!actor)
        return;
    const [[target]] = await pool.query("SELECT id FROM users WHERE id = ? LIMIT 1", [req.params.userId]);
    if (!target)
        return sendError(res, 404, "用户不存在");
    const [[badge]] = await pool.query("SELECT id, name, badge_type, achievement_points FROM legendary_badges WHERE id = ? LIMIT 1", [req.params.badgeId]);
    if (!badge)
        return sendError(res, 404, "传说徽章不存在");
    if (String(badge.badge_type) !== "limited")
        return sendError(res, 403, "只有限定徽章支持管理员直接发放");
    const key = `legendary:${badge.id}`;
    const result = await grantBadge({
...
app.delete("/api/admin/badges/users/:userId/legendary/:badgeId", async (req, res) => {
    if (!(await requireAdmin(req, res)))
        return;
    const [result] = await pool.query("DELETE FROM user_badge_unlocks WHERE user_id = ? AND badge_key = ?", [req.params.userId, `legendary:${req.params.badgeId}`]);
    if (result.affectedRows === 0)
        return sendError(res, 404, "该用户未拥有此传说徽章");
    await pool.query("UPDATE users SET equipped_badge_key = NULL, equipped_badge_icon_url = NULL WHERE id = ? AND equipped_badge_key = ?", [req.params.userId, `legendary:${req.params.badgeId}`]);
    res.json({ ok: true });
});
app.post("/api/me/feedback", async (req, res) => {
    ...
    if (!parsed.success)
        return sendError(res, 400, parsed.error.issues[0]?.message ?? "意见内容不正确");
    const id = nanoid();
    let screenshot = null;
    if (parsed.data.screenshot) {
        screenshot = await optimizeFeedbackScreenshot(parsed.data.screenshot, id);
        if (!screenshot)
            return sendError(res, 400, "截图无效、格式不支持或压缩后仍超过 2MB");
    }
    await pool.query(`INSERT INTO user_feedback
       (id, user_id, publisher_name, publisher_username, title, feedback_type, content, screenshot)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [id, user.id, user.nickname, user.username, parsed.data.title, parsed.data.type, parsed.data.content, screenshot]);
...
app.get("/api/admin/module-unread", async (req, res) => {
    ...
    const [eventRows, readRows] = await Promise.all([
        pool.query("SELECT module_key, MAX(id) AS newest_event_id FROM admin_module_events WHERE module_key IN ('approvals', 'feedback') GROUP BY module_key").then(([rows]) => rows),
        pool.query("SELECT module_key, last_event_id FROM admin_module_reads WHERE module_key IN ('approvals', 'feedback')").then(([rows]) => rows)
    ]);
...
app.patch("/api/admin/module-unread/:module/read", async (req, res) => {
    ...
    if (moduleKey !== "approvals" && moduleKey !== "feedback")
        return sendError(res, 400, "后台模块不正确");
    await pool.query(`INSERT INTO admin_module_reads (module_key, last_read_at, last_event_id, updated_by)
     ...
     ON DUPLICATE KEY UPDATE
       ...
       updated_by = VALUES(updated_by)`, [moduleKey, admin.id, moduleKey]);
    for (const adminId of await backofficeAdminIds()) {
...
app.get("/api/admin/feedback", async (req, res) => {
    ...
    const [rows] = await pool.query(`SELECT f.id, f.user_id, f.publisher_name, f.publisher_username, f.title, f.feedback_type, f.content, f.created_at
     FROM user_feedback f
     ${where}
     ORDER BY f.created_at ${order}, f.id ${order}
     LIMIT 10 OFFSET ?`, [...params, offset]);
    const [[totalRow]] = await pool.query(`SELECT COUNT(*) AS total FROM user_feedback f ${where}`, params);
...
app.get("/api/admin/feedback/:id", async (req, res) => {
    if (!(await requireBackofficeAdmin(req, res)))
        return;
    const [rows] = await pool.query(`SELECT id, user_id, publisher_name, publisher_username, title, feedback_type, content, screenshot, created_at
     FROM user_feedback WHERE id = ? LIMIT 1`, [req.params.id]);
    if (!rows[0])
        return sendError(res, 404, "建议不存在");
    res.json({ feedback: feedbackPayload(rows[0], true) });
});
app.post("/api/admin/notices/images", express.raw({ type: ["image/png", "image/jpeg", "image/webp", "image/gif"], limit: "2mb" }), async (req, res) => {
    if (!(await requireAdmin(req, res)))
        return;
    if (!Buffer.isBuffer(req.body) || req.body.length === 0)
        return sendError(res, 400, "图片文件无效");
    try {
...
app.post("/api/admin/android-releases", async (req, res) => {
    ...
    if (!parsed.success)
        return sendError(res, 400, parsed.error.issues[0]?.message ?? "Android 发布信息不正确");
    const id = nanoid();
    try {
        await pool.query(`INSERT INTO android_app_releases
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [
        ...
        if (error.code === "ER_DUP_ENTRY")
            return sendError(res, 409, "该 versionCode 已存在");
        throw error;
    }
    res.status(201).json({ id });
});
app.patch("/api/admin/android-releases/:id/enabled", async (req, res) => {
    ...
    if (!parsed.success)
        return sendError(res, 400, "enabled 必须是布尔值");
    const [result] = await pool.query("UPDATE android_app_releases SET enabled = ? WHERE id = ?", [parsed.data.enabled ? 1 : 0, req.params.id]);
    if (result.affectedRows === 0)
        return sendError(res, 404, "Android 发布版本不存在");
    res.json({ ok: true });
});
app.get("/api/admin/notices", async (req, res) => {
    ...
    const [rows] = await pool.query(`SELECT n.id, n.title, n.author, n.published_at, n.updated_at, n.expires_at, n.valid_duration_minutes,
     ...
     ORDER BY n.published_at DESC, n.id DESC
     LIMIT ? OFFSET ?`, [...params, limit, offset]);
    const [[totalRow]] = await pool.query(`SELECT COUNT(*) AS total FROM admin_notices n ${where}`, params);
...
app.post("/api/admin/notices", async (req, res) => {
    ...
    if (!parsed.success)
        return sendError(res, 400, parsed.error.issues[0]?.message ?? "通知内容不正确");
    const id = nanoid();
    const validDurationMinutes = parsed.data.validDays * 1440 + parsed.data.validHours * 60;
    await pool.query(`INSERT INTO admin_notices
       (id, title, author, content, created_by, valid_duration_minutes, expires_at)
     VALUES (?, ?, ?, ?, ?, ?, DATE_ADD(CURRENT_TIMESTAMP, INTERVAL ? MINUTE))`, [id, parsed.data.title, parsed.data.author, parsed.data.content, admin.id, validDurationMinutes, validDurationMinutes]);
...
app.post("/api/admin/notices/bulk-delete", async (req, res) => {
    ...
    if (!parsed.success)
        return sendError(res, 400, "请选择要删除的通知");
    const ids = [...new Set(parsed.data.ids)];
    const placeholders = ids.map(() => "?").join(",");
    const [result] = await pool.query(`DELETE FROM admin_notices WHERE id IN (${placeholders})`, ids);
    if (result.affectedRows > 0)
...
app.get("/api/admin/notices/:id/readers", async (req, res) => {
    if (!(await requireAdmin(req, res)))
        return;
    const [noticeRows] = await pool.query("SELECT id, title FROM admin_notices WHERE id = ? LIMIT 1", [req.params.id]);
    if (!noticeRows[0])
        return sendError(res, 404, "通知不存在");
    const [rows] = await pool.query(`SELECT u.id, u.nickname, u.username, nr.read_at
     FROM admin_notice_reads nr
     INNER JOIN users u ON u.id = nr.user_id
     WHERE nr.notice_id = ?
     ORDER BY nr.read_at DESC`, [req.params.id]);
    res.json({
...
app.get("/api/admin/notices/:id", async (req, res) => {
    ...
    const [rows] = await pool.query(`SELECT n.*,
       (SELECT COUNT(*) FROM admin_notice_reads nr WHERE nr.notice_id = n.id) AS read_count
     FROM admin_notices n WHERE n.id = ? LIMIT 1`, [req.params.id]);
    if (!rows[0])
        return sendError(res, 404, "通知不存在");
    if (req.query.trackRead !== "false") {
        await pool.query("INSERT IGNORE INTO admin_notice_reads (notice_id, user_id) VALUES (?, ?)", [req.params.id, admin.id]);
    }
    const [[countRow]] = await pool.query("SELECT COUNT(*) AS read_count FROM admin_notice_reads WHERE notice_id = ?", [req.params.id]);
...
app.put("/api/admin/notices/:id", async (req, res) => {
    ...
    if (!parsed.success)
        return sendError(res, 400, parsed.error.issues[0]?.message ?? "通知内容不正确");
    const validDurationMinutes = parsed.data.validDays * 1440 + parsed.data.validHours * 60;
    const [result] = await pool.query(`UPDATE admin_notices
     SET title = ?, author = ?, content = ?, valid_duration_minutes = ?,
         expires_at = DATE_ADD(published_at, INTERVAL ? MINUTE)
     WHERE id = ?`, [parsed.data.title, parsed.data.author, parsed.data.content, validDurationMinutes, validDurationMinutes, req.params.id]);
    if (result.affectedRows === 0)
        return sendError(res, 404, "通知不存在");
...
app.delete("/api/admin/notices/:id", async (req, res) => {
    if (!(await requireAdmin(req, res)))
        return;
    const [result] = await pool.query("DELETE FROM admin_notices WHERE id = ?", [req.params.id]);
    if (result.affectedRows === 0)
        return sendError(res, 404, "通知不存在");
    await broadcastUnreadChanged("notice_deleted");
    res.json({ ok: true });
...
app.get("/api/admin/users", async (req, res) => {
    ...
    const onlineOrderClause = prioritizeOnline
        ? onlineUserIds.length > 0
            ...
            : ""
        : "";
    const [rows] = await pool.query(`SELECT u.id, u.username, u.nickname, u.avatar, u.role, u.created_at, u.last_login_at, u.shell_balance,
     ...
     LIMIT ? OFFSET ?`, [...params, ...onlineUserIds, limit, offset]);
    const [[totalRow]] = await pool.query(`SELECT COUNT(*) AS total FROM users u ${where}`, params);
    res.json({
        total: Number(totalRow.total ?? 0),
        users: rows.map((row) => ({
...
app.patch("/api/admin/users/:id", async (req, res) => {
    ...
    if (!parsed.success)
        return sendError(res, 400, "用户信息不正确");
    ...
    const [targetRows] = await pool.query("SELECT role FROM users WHERE id = ? LIMIT 1", [req.params.id]);
    if (!targetRows[0])
        return sendError(res, 404, "用户不存在");
...
app.delete("/api/admin/users/:id", async (req, res) => {
    ...
    if (!user)
        return;
    if (user.id === req.params.id)
        return sendError(res, 400, "不能删除自己");
...
app.post("/api/admin/users/:id/reset-password", async (req, res) => {
    ...
    await pool.query("UPDATE password_reset_tokens SET consumed_at = UTC_TIMESTAMP() WHERE user_id = ? AND consumed_at IS NULL", [req.params.id]);
    res.json({ ok: true });
});
app.get("/api/admin/users/:id/shell-transactions", async (req, res) => {
    if (!(await requireBackofficeAdmin(req, res)))
        return;
    const [[userRow]] = await pool.query("SELECT id, shell_balance FROM users WHERE id = ? LIMIT 1", [req.params.id]);
    if (!userRow)
        return sendError(res, 404, "用户不存在");
    const limit = Math.min(50, Math.max(1, Math.floor(Number(req.query.limit ?? 20) || 20)));
    const offset = Math.max(0, Math.floor(Number(req.query.offset ?? 0) || 0));
...
app.post("/api/admin/users/bulk-shell-adjustments/preview", async (req, res) => {
    ...
    if (!parsed.success)
        return sendError(res, 400, parsed.error.issues[0]?.message ?? "批量贝壳条件无效");
    ...
    if (parsed.data.operation === "deduct" && userIds.length > 0) {
        ...
        const [[row]] = await pool.query(`SELECT COUNT(*) AS count FROM users WHERE id IN (${placeholders}) AND shell_balance >= ?`, [...userIds, parsed.data.amount]);
...
app.post("/api/admin/users/bulk-shell-adjustments", async (req, res) => {
    ...
    for (const userId of result.adjustedUserIds)
        emitUnreadChanged(userId, "shell_adjustment");
...
app.post("/api/admin/users/:id/shell-adjustments", async (req, res) => {
    ...
    if (!parsed.success)
        return sendError(res, 400, "请输入有效的贝壳整数数量");
    try {
        ...
        if (error instanceof Error && error.message === "SHELL_USER_NOT_FOUND")
            return sendError(res, 404, "用户不存在");
        if (error instanceof Error && error.message === "SHELL_INSUFFICIENT_BALANCE")
            return sendError(res, 409, "扣减数量不能超过当前贝壳余额");
        throw error;
    }
});
app.post("/api/admin/users/:id/experience-adjustments", async (req, res) => {
    ...
    if (!parsed.success)
        return sendError(res, 400, "请输入有效的经验整数数量");
    const connection = await pool.getConnection();
    try {
        await connection.beginTransaction();
        const [[userRow]] = await connection.query("SELECT experience FROM users WHERE id = ? FOR UPDATE", [req.params.id]);
        if (!userRow)
            throw new Error("EXPERIENCE_USER_NOT_FOUND");
        ...
        await connection.query(`INSERT INTO user_experience_adjustments
        (id, user_id, admin_id, amount, experience_after)
       VALUES (?, ?, ?, ?, ?)`, [nanoid(), req.params.id, admin.id, signedAmount, experience]);
        await connection.commit();
        res.json({ experience, level: levelForExperience(experience) });
    }
    catch (error) {
        await connection.rollback();
        if (error instanceof Error && error.message === "EXPERIENCE_USER_NOT_FOUND")
            return sendError(res, 404, "用户不存在");
        if (error instanceof Error && error.message === "EXPERIENCE_INSUFFICIENT")
            return sendError(res, 409, "扣除数量不能超过用户当前经验");
        if (error instanceof Error && error.message === "EXPERIENCE_MAX_EXCEEDED")
            return sendError(res, 409, `用户经验不能超过 ${MAX_EXPERIENCE.toLocaleString()}`);
...
app.patch("/api/admin/evaluations/:id", async (req, res) => {
    ...
    if (!parsed.success)
        return sendError(res, 400, "评分必须在 1-5 之间，步长 0.5；维度评分必须在 0-5 之间");
    const data = parsed.data;
    const [result] = await pool.query(`UPDATE evaluations
    ...
    if (result.affectedRows === 0)
        return sendError(res, 404, "评价不存在");
    const [rows] = await pool.query(`SELECT e.*, s.title AS soup_title
     FROM evaluations e
...
registerGiftRoutes(app, {
    ...
    onCircleGift: async (circleId, messageId, senderId) => {
       ...
       WHERE m.id = ? LIMIT 1`, [messageId]);
        ...
        const [members] = await pool.query("SELECT user_id FROM circle_members WHERE circle_id = ? AND user_id <> ?", [circleId, senderId]);
        for (const member of members) {
    ...
    try {
        ...
        if (url.pathname === "/ws/circles") {
            ...
            const [[user], [members]] = await Promise.all([
                pool.query("SELECT id, token_version FROM users WHERE id = ? LIMIT 1", [claims.id]),
                pool.query("SELECT user_id FROM circle_members WHERE circle_id = ? AND user_id = ? LIMIT 1", [circleId, claims.id])
            ]);
            if (!user[0] || Number(user[0].token_version ?? 0) !== claims.tokenVersion || !members[0])
                return socket.destroy();
            onlineSoupWebSocketServer.handleUpgrade(request, socket, head, (webSocket) => {
                webSocket.circleUserId = claims.id;
                webSocket.circleId = circleId;
        ...
        if (url.pathname === "/ws/online-soup-lobby") {
            onlineSoupWebSocketServer.handleUpgrade(request, socket, head, (webSocket) => {
                webSocket.onlineSoupLobby = true;
                onlineSoupWebSocketServer.emit("connection", webSocket, request);
            });
            return;
        }
        if (url.pathname !== "/ws/online-soup")
            return socket.destroy();
        ...
        const [[user], [members]] = await Promise.all([
            pool.query("SELECT id, token_version FROM users WHERE id = ? LIMIT 1", [claims.id]),
            pool.query(`SELECT m.user_id, r.host_id FROM online_soup_members m
         JOIN online_soup_rooms r ON r.id = m.room_id
         WHERE m.room_id = ? AND m.user_id = ? AND m.is_active = 1 AND r.status <> 'closed' LIMIT 1`, [roomId, claims.id])
        ]);
        if (!user[0] || Number(user[0].token_version ?? 0) !== claims.tokenVersion || !members[0])
            return socket.destroy();
        onlineSoupWebSocketServer.handleUpgrade(request, socket, head, (webSocket) => {
            webSocket.onlineSoupUserId = claims.id;
            webSocket.onlineSoupRoomId = roomId;
            webSocket.onlineSoupIsHost = String(members[0].host_id) === String(claims.id);
            onlineSoupWebSocketServer.emit("connection", webSocket, request);
        });
    }
    catch {
...
onlineSoupWebSocketServer.on("connection", (socket) => {
    if (socket.circleId) {
        const userId = String(socket.circleUserId);
        const circleId = String(socket.circleId);
        const clients = circleSocketClients.get(circleId) ?? new Set();
        clients.add(socket);
        ...
        socket.on("close", () => {
            ...
            if (clients.size === 0)
                circleSocketClients.delete(circleId);
            unregisterPresenceConnection(userId);
        });
        return;
    }
    if (socket.onlineSoupLobby) {
    ...
    const userId = String(socket.onlineSoupUserId);
    const roomId = String(socket.onlineSoupRoomId);
    let lastPresencePersistedAt = Date.now();
    const clients = onlineSoupRoomSocketClients.get(roomId) ?? new Set();
    clients.add(socket);
    ...
    socket.on("message", (raw) => {
        try {
            ...
            if (message?.type !== "ping")
                return;
            socket.send(JSON.stringify({ event: "pong", payload: { at: new Date().toISOString() } }));
            if (Date.now() - lastPresencePersistedAt < 60_000)
                return;
            lastPresencePersistedAt = Date.now();
            void pool.query("UPDATE online_soup_members SET last_seen_at = NOW() WHERE room_id = ? AND user_id = ?", [roomId, userId]);
            void pool.query("UPDATE online_soup_rooms SET host_last_seen_at = NOW() WHERE id = ? AND host_id = ? AND host_grace_started_at IS NULL", [roomId, userId]);
        }
        catch { /* Ignore malformed client frames. */ }
    });
    socket.on("close", () => {
        clients.delete(socket);
        if (clients.size === 0)
            onlineSoupRoomSocketClients.delete(roomId);
        unregisterPresenceConnection(userId);
    });
});
