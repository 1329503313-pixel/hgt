# 汤物语 Android APP 视觉、图标与头像对齐执行说明
...
新的 `apps/app-android` 不再复用本节所审计的旧 uni-app 页面，而是直接打包 Web 手机用户端的共享 `UserApp`。应用短名称统一为“汤物语”，完整品牌名称为“汤物语丨汤汤解谜乐园”；Launcher 图标从既有侦探海龟品牌图机械生成，启动页复用无文字的侦探海龟画面；Android 各密度资源及来源 SHA256 已纳入门禁。Lucide、头像、等级、徽章、底部导航和全部业务页面因此继续以 Web 实现为唯一真源，不再维护第二套近似组件。
