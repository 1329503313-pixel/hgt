...
export default function OnlineSoupRoomPage() {
  ...
  const [entryRetryError, setEntryRetryError] = useState<string | null>(null);
        ...
        try {
          ...
          if (!quietPass && error instanceof ApiError && error.code === "NOT_MEMBER") {
            try {
              ...
              } else if (isTerminalOnlineSoupJoinError(joinError)) {
                setEntryError(isClosedOnlineSoupInvite(joinError) ? "房间不存在或已关闭" : "房间已满");
              } else {
                setEntryRetryError(joinError instanceof Error ? joinError.message : "加入房间失败");
              }
            }
          } else if (!quietPass) {
            setEntryError(isClosedOnlineSoupInvite(error) ? "房间不存在或已关闭" : null);
            if (!isClosedOnlineSoupInvite(error)) {
              setEntryRetryError(error instanceof Error ? error.message : "房间加载失败");
            }
          }
        }
        quietPass = true;
  ...
  useEffect(() => connectOnlineSoupSocket(roomId, (reason, payload) => {
    ...
    if (reason === "best_question_changed") {
      const bestQuestionMessageId = typeof payload.bestQuestionMessageId === "string" ? payload.bestQuestionMessageId : null;
      setSnapshot((current) => current ? {
        ...current,
        room: { ...current.room, bestQuestionMessageId },
        messages: current.messages.map((message) => ({ ...message, isBestQuestion: message.id === bestQuestionMessageId }))
      } : current);
      setProgressQuestions((current) => current.map((question) => ({ ...question, isBestQuestion: question.id === bestQuestionMessageId })));
      if (payload.activityType === "progress") void loadProgress(true);
      if (payload.notificationCreated) void loadNewMessages();
      return;
    }
  ...
  async function toggleBestQuestion(message: OnlineSoupMessage) {
    if (bestQuestionSavingId) return;
    setBestQuestionSavingId(message.id);
    try {
      const result = await api<{ bestQuestionMessageId: string | null }>(`/api/online-soup/rooms/${roomId}/questions/${message.id}/best-question`, {
        method: "PATCH",
        body: { selected: !message.isBestQuestion }
      });
      const bestQuestionMessageId = result.bestQuestionMessageId;
      setSnapshot((current) => current ? {
        ...current,
        room: { ...current.room, bestQuestionMessageId },
        messages: current.messages.map((item) => ({ ...item, isBestQuestion: item.id === bestQuestionMessageId }))
      } : current);
      setProgressQuestions((current) => current.map((question) => ({ ...question, isBestQuestion: question.id === bestQuestionMessageId })));
    } catch (error) {
      showToast(error instanceof Error ? error.message : "最佳提问设置失败");
    } finally {
      setBestQuestionSavingId("");
    }
  }
  ...
  async function manageMember(action: "kick" | "transfer" | "mute" | "unmute", durationMinutes?: 1 | 5) {
    ...
    try {
      const endpoint = action === "transfer" ? "transfer-host" : action;
      await api(`/api/online-soup/rooms/${roomId}/members/${managedMemberId}/${endpoint}`, {
        method: "POST",
        ...(action === "mute" ? { body: { durationMinutes } } : {})
      });
      if (action === "kick" || action === "transfer") setManagedMemberId(null);
      setMemberManagementAction(null);
      showToast(action === "kick" ? "已将该用户踢出房间" : action === "transfer" ? "房主已转让" : action === "mute" ? `已禁言 ${durationMinutes} 分钟` : "已取消禁言");
      await Promise.all([loadState(), loadNewMessages()]);
    } catch (error) {
  ...
  if (loading || !snapshot) return <div className="min-h-screen bg-page p-4 pt-24">
    ...
    {entryRetryError && !entryPasswordOpen && !entryError && <Modal onClose={returnFromInvite}>
      <div className="space-y-4 text-center">
        <div><h2 className="text-xl font-black text-ink">进入房间失败</h2><p className="mt-2 text-sm leading-6 text-muted">{entryRetryError}</p></div>
        <p className="text-xs leading-5 text-muted">二维码邀请仍然有效，请检查网络后重新尝试。</p>
        <div className="grid grid-cols-2 gap-2"><button className="btn btn-secondary" onClick={returnFromInvite}>返回大厅</button><button className="btn btn-primary" onClick={retryEntry}>重新进入</button></div>
      </div>
    </Modal>}
  </div>;
    ...
    <div className="online-soup-room-theme flex h-[100dvh] flex-col overflow-hidden bg-page">
      ...
      <main className={`online-soup-room-workspace mx-auto grid min-h-0 w-full flex-1 grid-rows-[auto_minmax(0,1fr)] gap-2 overflow-hidden px-4 pb-3 pt-3 lg:grid-rows-1 lg:gap-4 lg:px-6 lg:pb-5 lg:pt-5 ${showRoomActions ? "max-w-[1480px] lg:grid-cols-[340px_minmax(0,1fr)_76px_76px]" : "max-w-[1388px] lg:grid-cols-[340px_minmax(0,1fr)_76px]"}`}>
        <aside className="flex max-h-[30dvh] min-h-0 flex-col gap-3 overflow-hidden lg:order-1 lg:max-h-none">
          ...
          {!mysteryMode && !impostorMode && <section className={`card flex min-h-0 flex-col overflow-hidden ${soupExpanded && snapshot.room.soup ? "flex-1" : "shrink-0"}`}>
        ...
        <section className={`card relative flex min-h-0 flex-col overflow-hidden lg:order-2 ${impostorNightMode ? "impostor-night-chat" : ""}`}>
          ...
          {canParticipate && currentMemberMuted && <div className="shrink-0 border-t border-red-100 bg-red-50 px-4 py-3 pb-[max(12px,env(safe-area-inset-bottom))] text-center text-sm font-bold text-red-600" role="status"><VolumeX className="mr-1.5 inline" size={16} />你已被房主禁言，暂时无法发送消息</div>}
        </section>
      </main>
    ...
    <div className="mt-2 space-y-1.5 pl-10">
      {question.answer
        ...
        : statusText
            ...
            : null}
      {question.isBestQuestion && <span className="ml-1.5 inline-flex items-center gap-1 rounded-full border border-amber-500 bg-amber-400 px-2.5 py-1 text-xs font-black text-amber-950"><Star size={11} fill="currentColor" />最佳提问</span>}
  ...
  return (
    <div className={`flex items-start gap-2.5 ${mine ? "flex-row-reverse" : ""}`}>
      ...
      >
        ...
        {senderMuted && <MutedAvatarIndicator />}
      </MentionableAvatarButton>
      <div className={`flex min-w-0 max-w-[78%] flex-col ${question ? "w-full max-w-[84%]" : ""} ${mine ? "items-end" : "items-start"}`}>
          ...
          <div className={`mt-2 max-w-full rounded-xl border border-violet-200 bg-violet-50 px-2.5 py-2 ${mine ? "text-right" : "text-left"}`}>
            {isHost ? (
              <div className={`flex flex-wrap gap-1.5 ${mine ? "justify-end" : ""}`}>
                ...
                <button
                  type="button"
                  className={`inline-flex min-h-11 items-center gap-1 rounded-full border px-3 text-xs font-black transition disabled:cursor-wait disabled:opacity-60 ${message.isBestQuestion ? "border-amber-500 bg-amber-400 text-amber-950 shadow-sm" : "border-amber-300 bg-white text-amber-700 hover:border-amber-500 hover:bg-amber-50"}`}
                  disabled={bestQuestionSaving}
                  aria-pressed={message.isBestQuestion}
                  aria-label={message.isBestQuestion ? `取消第${message.questionNumber}题的最佳提问标记` : `将第${message.questionNumber}题标记为最佳提问`}
                  title={message.isBestQuestion ? "点击取消最佳提问" : "标记为最佳提问"}
                  onClick={() => onToggleBestQuestion(message)}
                >
                  {bestQuestionSaving ? <LoaderCircle size={14} className="animate-spin" /> : <Star size={14} fill={message.isBestQuestion ? "currentColor" : "none"} />}
                  最佳提问
                </button>
              </div>
            ) : message.answer ? (
        ...
        {question && message.isBestQuestion && !isHost && (
          <div className={`mt-1.5 max-w-full ${mine ? "text-right" : "text-left"}`}>
            <span className="inline-flex items-center gap-1 rounded-full border border-amber-500 bg-amber-400 px-2.5 py-1 text-xs font-black text-amber-950"><Star size={12} fill="currentColor" />最佳提问</span>
          </div>
        )}
