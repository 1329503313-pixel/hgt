...
import { Crown, Flame, GalleryVerticalEnd, Medal, Sparkles, Trophy } from "lucide-react";
import { api } from "../api";
...
type RankingTab = "soups" | "users" | "collection";
...
export default function RankingsPage() {
  ...
  const podium = tab === "soups"
    ...
    : tab === "users"
      ? (data?.achievementUsers ?? []).slice(0, 3).map((item) => ({ id: item.id, rank: item.rank, name: item.nickname, detail: "社区成就", value: item.achievementPoints, suffix: "成就点", avatar: null as string | null }))
  ...
  const ownRank = tab === "soups" ? data?.hotSoupOwn : tab === "users" ? data?.achievementOwn : data?.collectionOwn;
  ...
  return (
    <section className="rankings-page space-y-3 lg:space-y-5">
      ...
      <div className="rankings-category-panel card p-2 lg:p-4">
        ...
        <div className="rankings-category-grid grid grid-cols-3 gap-2 lg:mt-4">
      ...
      <div className="rankings-workspace">
        <aside className={`rankings-spotlight hidden lg:flex ${activeCategory.tone}`}>
              ...
              {podium.map((item) => (
                <button key={`${tab}-${item.id}`} type="button" className={`rankings-podium-item is-rank-${item.rank}`} onClick={() => openPodiumItem(item)}>
                  <span className="rankings-podium-avatar">
                    {item.avatar ? <img src={item.avatar} alt="" /> : tab === "soups" ? <Flame size={24} /> : item.name.slice(0, 1)}
          ...
          <div className="rankings-rule-card">
            <Sparkles size={17} />
            <div><strong>{activeCategory.label}统计口径</strong><p>{activeCategory.description}。{tab === "soups" ? "按平台热力值从高到低排列。" : tab === "collection" ? "同分时，先达到当前收藏值的用户优先。" : "同分时，先达到当前成就点的用户优先。"}</p></div>
          </div>
