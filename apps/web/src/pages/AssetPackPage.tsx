  ...
  return (
    <section className="min-h-screen bg-page">
      ...
      <div className="asset-pack-detail-layout mx-auto max-w-6xl space-y-4 px-4 pb-32 lg:space-y-0">
        ...
        {Boolean(pack.collectibleRewards?.length) && <section className="card p-4" aria-labelledby="pack-collectibles-title"><div className="flex items-center justify-between gap-3"><div><h2 id="pack-collectibles-title" className="font-black text-ink">关联收藏品</h2><p className="mt-1 text-xs leading-5 text-muted">每件收藏品独立判定；已被获取的收藏品不会再次抽出</p></div><Gem className="shrink-0 text-amber-600" size={24}/></div><div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-4">{pack.collectibleRewards!.map(item=><button type="button" key={item.id} className="min-h-11 rounded-2xl text-left transition hover:ring-2 hover:ring-amber-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" onClick={()=>navigate(`/mine/collectibles/${item.id}`,{state:{backTo:`/mine/store/cards/${pack.id}`}})} aria-label={`查看收藏品${item.name}详情`}><CollectibleVisual collectible={item} className="aspect-[5/6]"/><span className={`mt-2 block text-center text-xs font-black ${item.acquired?"text-muted":"text-emerald-700"}`}>{item.acquired?"已被获取":"未被获取"}</span><span className="mt-1 block text-center text-[11px] font-bold text-muted">独立概率 {item.probability}%</span></button>)}</div></section>}
