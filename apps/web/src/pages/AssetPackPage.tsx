    ...
    <section className="min-h-screen bg-page">
      ...
      <div className="asset-pack-detail-layout mx-auto max-w-6xl space-y-4 px-4 pb-32 lg:space-y-0">
        ...
        <div className="asset-pack-detail-content space-y-4">
          {Boolean(pack.collectibleRewards?.length) && <section className="card p-4" aria-labelledby="pack-collectibles-title"><div className="flex items-center justify-between gap-3"><div><h2 id="pack-collectibles-title" className="font-black text-ink">关联收藏品</h2><p className="mt-1 text-xs leading-5 text-muted">每件收藏品独立判定；每累计 100 抽，当前用户概率增加 0.01 个百分点，最多增加 0.04 个百分点</p></div><Gem className="shrink-0 text-amber-600" size={24}/></div><div className="mt-3 rounded-xl bg-amber-50 px-3 py-2 text-xs font-bold leading-5 text-amber-800">抽数按用户和当前卡包独立累计；已被获取的收藏品不会再次抽出。</div><div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-4">{pack.collectibleRewards!.map(item=><button type="button" key={item.id} className="min-h-11 cursor-pointer rounded-2xl text-left transition hover:ring-2 hover:ring-amber-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" onClick={()=>navigate(`/mine/collectibles/${item.id}`,{state:{backTo:`/mine/store/cards/${pack.id}`}})} aria-label={`查看收藏品${item.name}详情，${item.acquired?"已被获取":`当前独立概率${item.probability}%`}`}><CollectibleVisual collectible={item} className="aspect-[5/6]"/><span className={`mt-2 block text-center text-xs font-black ${item.acquired?"text-muted":"text-emerald-700"}`}>{item.acquired?"已被获取":"未被获取"}</span><span className="mt-1 block text-center text-[11px] font-bold text-muted">{item.acquired?"抽中概率":"当前独立概率"} {item.probability}%</span>{!item.acquired&&Boolean(item.probabilityBoost)&&<span className="mt-0.5 block text-center text-[11px] font-bold text-amber-700">基础 {item.baseProbability}% ＋ 加成 {item.probabilityBoost}%</span>}</button>)}</div></section>}
