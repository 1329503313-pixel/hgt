...
export default function ResetPasswordPage() {
  ...
  if (loadingUser) return <section className="space-y-4"><PageTopBar title="重置密码" /><MineBackButton /><CardSkeleton rows={5} /></section>;
  if (!user) return <section className="space-y-4"><PageTopBar title="重置密码" /><MineBackButton /><div className="card p-6 text-center"><p className="text-sm text-muted">登录后重置密码</p><button className="btn btn-primary mt-4 w-full" onClick={openAuth}>登录</button></div></section>;
