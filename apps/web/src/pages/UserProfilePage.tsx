  ...
  return (
    <section className="user-profile-page min-h-screen bg-page pt-[72px]">
      ...
      <div className="user-profile-content mx-auto max-w-3xl space-y-3 px-4 pb-10">
        ...
        <ProfileHero key={`${profile.id}:${profile.profileBackgroundSourceUrl ?? "default"}`} className="user-profile-hero" profile={profile} collapsibleBackground={!profile.isSelf} onFollowing={() => navigate(`/users/${profile.id}/following`)} onFollowers={() => navigate(`/users/${profile.id}/followers`)} onCharm={!profile.isSelf ? () => setGiftOpen(true) : undefined} actions={!profile.isSelf ? (
