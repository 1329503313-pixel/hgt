      ...
      {editorMode === "source" ? <>
        ...
        <section className="min-w-0 space-y-4">
          {selectedVersion ? <>
            ...
            <Field label="结构化 Story Package JSON" hint="保存前由服务端执行完整 Schema 校验；已发布版本不可覆盖，只能重新编译新版本。"><textarea spellCheck={false} className="field min-h-[800dvh] w-full resize-y font-mono text-xs leading-5" value={packageText} onChange={(event) => setPackageText(event.target.value)} readOnly={Boolean(selectedVersion.publishedAt)} /></Field>
