...
export function VipManagement() {
  ...
  return <div className="space-y-4">
    ...
    {tab === "orders" ? <VipOrders /> : <div className="card p-4">
      ...
      {error && <p className="mb-3 rounded-xl bg-red-50 p-3 text-sm font-bold text-red-600" role="alert">{error}</p>}
      <div className="overflow-x-auto"><div className="min-w-[1050px]"><div className="grid grid-cols-[150px_150px_130px_220px_110px_290px] gap-2 px-3 pb-2 text-center text-xs font-bold text-muted"><span>用户昵称</span><span>用户账号</span><span>当前身份</span><span>剩余VIP时间</span><span>已过期时间</span><span>操作</span></div>{!loading && <div className="space-y-1">{users.map((user) => { const vipActive = user.legacyActive || Boolean(user.vipExpiresAt && new Date(user.vipExpiresAt).getTime() > Date.now()); return <div key={user.id} className="grid grid-cols-[150px_150px_130px_220px_110px_290px] items-center gap-2 rounded-lg border border-line p-3 text-center text-sm"><span className="truncate font-semibold text-ink" title={user.nickname}>{user.nickname}</span><span className="truncate text-muted" title={user.username}>@{user.username}</span><span className={`mx-auto rounded-full px-2.5 py-1 text-xs font-black ${user.currentIdentity === "vip" ? "bg-amber-100 text-amber-800" : user.currentIdentity === "expired" ? "bg-slate-100 text-muted" : "bg-blue-50 text-primary"}`}>{IDENTITY_LABELS[user.currentIdentity]}</span><span className={`text-xs font-bold ${vipActive ? "text-emerald-700" : "text-muted"}`} title={user.vipExpiresAt ? `到期时间：${new Date(user.vipExpiresAt).toLocaleString()}` : undefined}>{formatRemainingMinutes(user.remainingMinutes, user.legacyActive, vipActive)}</span><span className="text-xs text-muted">{!vipActive && user.vipExpiresAt ? `已过期 ${user.expiredDays} 天` : "—"}</span><div className="flex items-center justify-center gap-1"><button type="button" className="btn btn-primary h-9 px-2 text-xs" onClick={() => setGrantUser(user)}><CalendarClock size={14} />赠送VIP</button><button type="button" className="btn btn-secondary h-9 px-2 text-xs text-red-600" onClick={() => setCancelUser(user)}><MinusCircle size={14} />取消VIP</button><button type="button" className="btn btn-secondary h-9 px-2 text-xs" onClick={() => setOrderUser(user)}><History size={14} />订单</button></div></div>; })}</div>}</div></div>
