  ...
  return <section className="card flex min-h-0 flex-col overflow-hidden">
    ...
    <div className="min-h-0 flex-1 space-y-3 overflow-y-auto overscroll-contain p-4">
      ...
      {game.phase === "night" && <>
        ...
        {!me ? <p className="text-sm text-muted">等待游戏者秘密行动，夜晚将在倒计时结束后统一结算。</p> : me.nightSubmitted ? <StatusNotice text="夜间行动已提交，夜晚将在倒计时结束后统一结算" /> : me.nightActionTypes.length === 0 ? <StatusNotice text="本夜没有可用技能，等待倒计时结束" /> : <>
...
export function ImpostorChatActionCard({ roomId, game, members, currentUserId, onChanged, showToast }: ChatActionProps) {
  ...
  useEffect(() => setSelectedTargets([]), [game?.phase, game?.nomination?.attempt, game?.accusation?.attempt]);

  if (!game || !["day_vote", "assassination", "accusation"].includes(game.phase)) return null;
  ...
  if (game.phase === "day_vote" && game.nomination) {
    ...
    return <article className="mx-auto mt-2 w-full max-w-xl rounded-2xl border border-violet-200 bg-violet-50 p-3 text-left shadow-sm" aria-label="任务人选投票">
      <div className="flex items-center gap-2 text-sm font-black text-violet-900"><Vote size={17} />{nomination.attempt > 1 ? `第 ${nomination.attempt} 次平票重投` : `选择 ${nomination.required} 名任务人选`}</div>
      ...
      {!game.me ? <StatusNotice text="旁观者不可参与投票" /> : submitted ? <StatusNotice text="任务人选投票已提交" /> : <>
        ...
        <button className="btn btn-primary mt-2 min-h-11 w-full" disabled={saving || selectedTargets.length !== nomination.required} onClick={() => void submit("impostor/nomination", { attempt: nomination.attempt, candidateUserIds: selectedTargets })}><Vote size={17} />{saving ? "提交中…" : "提交投票"}</button>
      </>}
  ...
  if (game.phase === "assassination") {
    return <article className="mx-auto mt-2 w-full max-w-xl rounded-2xl border border-rose-200 bg-rose-50 p-3 text-left shadow-sm" aria-label="伪人刺杀">
      <div className="flex items-center gap-2 text-sm font-black text-rose-900"><UserRoundSearch size={17} />伪人选择刺杀目标</div>
      {game.me?.canAssassinate ? <>
        ...
        <button className="btn mt-2 min-h-11 w-full bg-rose-600 text-white hover:bg-rose-700" disabled={saving || selectedTargets.length !== 1} onClick={() => void submit("impostor/assassinate", { targetUserId: selectedTargets[0] })}><UserRoundSearch size={17} />{saving ? "提交中…" : "确认刺杀"}</button>
  ...
  return <article className="mx-auto mt-2 w-full max-w-xl rounded-2xl border border-blue-200 bg-blue-50 p-3 text-left shadow-sm" aria-label="最终公投">
    <div className="flex items-center gap-2 text-sm font-black text-blue-900"><Vote size={17} />{accusation.attempt === 2 ? "平票重投：再次选择公投目标" : "所有玩家选择公投目标"}</div>
    {!game.me ? <StatusNotice text="旁观者不可参与公投" /> : submitted ? <StatusNotice text="公投目标已提交" /> : <>
      ...
      <div className="mt-2 grid grid-cols-2 gap-2"><button className="btn btn-secondary min-h-11" disabled={saving} onClick={() => void submit("impostor/accuse", { attempt: accusation.attempt, targetUserId: null })}>弃权</button><button className="btn btn-primary min-h-11" disabled={saving || selectedTargets.length !== 1} onClick={() => void submit("impostor/accuse", { attempt: accusation.attempt, targetUserId: selectedTargets[0] })}>{saving ? "提交中…" : "确认公投"}</button></div>
    </>}
    <SubmissionProgress submitted={accusation.submittedUserIds.length} total={game.playerSeats.length} />
  </article>;
}

function TargetGrid({ items, selected, playerName, onToggle }: { items: Array<{ userId: string; disabled?: boolean; suffix?: string }>; selected: string[]; playerName: (id: string) => string; onToggle: (id: string) => void }) {
  return <div className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-2">{items.map(({ userId, disabled = false, suffix = "" }) => <button key={userId} type="button" disabled={disabled} aria-pressed={selected.includes(userId)} className={`min-h-11 rounded-xl border px-3 py-2 text-left text-sm font-bold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 ${disabled ? "cursor-not-allowed border-slate-200 bg-slate-100 text-slate-400 opacity-80" : selected.includes(userId) ? "border-violet-500 bg-violet-600 text-white" : "border-line bg-white text-ink hover:border-violet-300 hover:bg-violet-50"}`} onClick={() => onToggle(userId)}>{playerName(userId)}{suffix}</button>)}</div>;
}
