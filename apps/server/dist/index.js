...
async function canViewFull(soup, user) {
    ...
    if (!user)
        return false;
    if (user.role === "admin" || user.id === soup.creator_id)
        return true;
...
async function notify(userId, type, title, content, relatedId) {
    await pool.query("INSERT INTO notifications (id, user_id, type, title, content, related_id) VALUES (?, ?, ?, ?, ?, ?)", [nanoid(), userId, type, title, content, relatedId]);
}
...
app.post("/api/auth/login", async (req, res) => {
    ...
    const [rows] = await pool.query("SELECT * FROM users WHERE username = ? LIMIT 1", [parsed.data.username]);
    const row = rows[0];
    if (!row)
...
app.post("/api/auth/password", async (req, res) => {
    ...
    const [rows] = await pool.query("SELECT id FROM users WHERE id = ? LIMIT 1", [user.id]);
    const row = rows[0];
    if (!row)
...
app.get("/api/soups/:id", async (req, res) => {
    ...
    LIMIT 1
    `, [req.params.id]);
    const [evalRows] = await pool.query("SELECT * FROM evaluations WHERE soup_id = ? ORDER BY created_at DESC", [req.params.id]);
...
app.post("/api/soups/:id/evaluations", async (req, res) => {
    ...
    const exists = await pool.query("SELECT id FROM evaluations WHERE soup_id = ? AND reviewer_id = ? LIMIT 1", [req.params.id, user.id]);
...
app.delete("/api/evaluations/:id", async (req, res) => {
    ...
    if (user.role !== "admin" && user.id !== evaluation.reviewer_id)
        return sendError(res, 403, "没有删除权限");
...
app.post("/api/access-requests/:id/decision", async (req, res) => {
    ...
    if (user.role !== "admin" && user.id !== request.owner_id)
