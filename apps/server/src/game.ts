...
answer 可以多写（2-4段），给方向性指引，但不透露汤底原文。
...
gameRouter.post("/:soupId/ask", async (req, res) => {
  ...
  const systemPrompt = buildSystemPrompt(soupData.surface, soupData.bottom, soupData.manual, soupData.supplementalSurfaces, soupData.supplementalBottoms, savedSupp.surfaces, savedSupp.bottoms, soupData.keyFacts, soupData.aiPrompt);
  ...
  await pool.query("UPDATE game_sessions SET messages = ?, revealed_supplements = ?, progress = ? WHERE id = ?",
    [JSON.stringify(fullMessages), JSON.stringify(mergedSupp), mergedProgress, session.id]);

  res.json({ answer: result.answer, progress: mergedProgress, revealedKeys: [], revealedSupplements: mergedSupp, completed: result.completed });
});
