...
人物不能只停留在静态介绍：每个填写 currentPlan 的 NPC 都必须通过 scheduleIds 关联至少一个能够推动该计划的世界事件，并为会改变主线、关键线索、重大风险或不可逆后果的核心反应配置可复用的行动转换。普通寒暄、拒绝、追问、姿态、无条件移动和现场互动由运行时通用行动规则处理，不要把转换图编译成穷尽式行动菜单。行动转换 description 只能描述玩家能够理解的尝试本身，不得写入成功结果、隐藏秘密或结局条件；玩家可执行转换的 precondition 必须引用玩家 actorId，NPC 专属转换必须引用对应 NPC actorId，便于运行时安全区分行动方向。
