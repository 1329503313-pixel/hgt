...
test("其他用户只有达到 3 级才计入作品评分", () => {
  ...
  assert.equal(MIN_SCORING_EXPERIENCE, 300);
  assert.equal(evaluationCountsTowardScore("reviewer", "creator", 299), false);
  assert.equal(evaluationCountsTowardScore("reviewer", "creator", 300), true);
});

test("评分聚合 SQL 同时排除上传者和低等级用户", () => {
  const sql = scoringEvaluationJoin("e", "s");
  assert.match(sql, /e\.reviewer_id <> s\.creator_id/);
  assert.match(sql, /scoring_reviewer\.experience >= 300/);
});
