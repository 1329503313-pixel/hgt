...
test("每日任务理论奖励为76，实际日上限为50", () => {
  const theoreticalMaximum = SHELL_TASKS.reduce((sum, task) => sum + task.reward * task.dailyLimit, 0);
  assert.equal(theoreticalMaximum, 76);
  assert.equal(SHELL_DAILY_LIMIT, 50);
});

test("任务奖励按剩余额度截断", () => {
  assert.equal(calculateTaskReward(0, 5), 5);
  assert.equal(calculateTaskReward(48, 5), 2);
  assert.equal(calculateTaskReward(50, 10), 0);
  assert.equal(calculateTaskReward(55, 3), 0);
});
