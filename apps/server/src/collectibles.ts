...
const collectibleSchema = z.object({
  ...
  rarity: z.enum(["limited", "collaboration", "legend", "epic"]),
...
export function startCollectibleAuctionScheduler(deps: Pick<Dependencies,"emitUserEvent"|"emitUnreadChanged"|"broadcastEvent">) {
  let running=false;
  const run=async()=>{if(running)return;running=true;try{
    ...
    for(const row of due){const connection=await pool.getConnection();let winner:string|null=null;try{await connection.beginTransaction();const [[a]]=await connection.query<mysql.RowDataPacket[]>("SELECT * FROM collectible_auctions WHERE id=? FOR UPDATE",[row.id]);if(!a||a.status!=="active"||new Date(a.ends_at)>new Date()){await connection.rollback();continue;}const [[item]]=await connection.query<mysql.RowDataPacket[]>("SELECT * FROM collectibles WHERE id=? FOR UPDATE",[a.collectible_id]);winner=a.highest_bidder_id?String(a.highest_bidder_id):null;if(winner){await connection.query("UPDATE collectible_auctions SET status='sold',settled_at=CURRENT_TIMESTAMP WHERE id=?",[a.id]);await connection.query("UPDATE collectibles SET status='owned',owner_user_id=? WHERE id=?",[winner,item.id]);await connection.query("INSERT INTO collectible_transfers (id,collectible_id,to_user_id,transfer_type,related_type,related_id,collectible_snapshot) VALUES (?,?,?,'auction','collectible_auction',?,?)",[nanoid(),item.id,winner,a.id,snapshot(item)]);await recordValueEvent(connection,item,winner,Number(item.collectible_value??1),"auction","collectible_auction",String(a.id));await insertNotification(connection,winner,"collectible_auction_won","竞拍成功",`你以 ${a.current_price} 贝壳拍得“${item.name}”`,String(a.id));}else{await connection.query("UPDATE collectible_auctions SET status='unsold',settled_at=CURRENT_TIMESTAMP WHERE id=?",[a.id]);await connection.query("UPDATE collectibles SET status='unowned' WHERE id=?",[item.id]);}await connection.commit();if(winner)deps.emitUnreadChanged(winner,"collectible_auction_won");deps.broadcastEvent("collectible_auction_changed",{auctionId:String(a.id),reason:winner?"sold":"unsold"});}catch{await connection.rollback();}finally{connection.release();}}
